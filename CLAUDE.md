...
- 構造化された質問ツール（GitHub Copilot CLI の ask_user、Claude Code の AskUserQuestion 等）は、選択肢へのクリック誤操作による即確定を防ぐ設定が環境で確認できる場合に限り使用すること。Claude Code では環境変数 `CLAUDE_CODE_DISABLE_MOUSE_CLICKS` が `1` に設定されていること（v2.1.195 以降で有効。セッション内で環境変数の値を出力して確認できる）。確認できた場合は、質問の重要度を問わず構造化質問ツールを使用してよい（ただし後述の事象確認済みモデルの条項に該当する場合は、本項によらず使用しないこと）
...
- 「構造化質問ツールと同一ターンに書いた説明テキストがユーザーに表示されない」事象が確認されているモデルで動作している場合は、質問の重要度・自己完結の可否を問わず**構造化質問ツールを使用しない**こと。すべての質問をテキストのみのターンで提示すること（選択肢を提示できる質問は説明と番号付き選択肢を添え、自由記述が必要な質問は前項の例外に従う）。本条件はセッション開始時に加え、モデル切替の通知を見た時点で再評価すること。動作モデルはシステムプロンプトの記載とモデル切替通知で確認し、判定できない場合は使用しない側に倒すこと（事象確認済みモデル: Claude Fable 5 — Claude Code の AskUserQuestion で再現、2026-06〜2026-08 に複数回確認。自己完結形式の質問でも本文非表示の事故が再発したため全面不使用とした。ユーザーから新たなモデルでの同事象の指摘があった場合はここに追記する。未確認のモデルでは、事象の発生を確認するまでこの対処には従わないこと）
