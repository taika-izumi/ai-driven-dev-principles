...
初期設計で判断の分担を合意した作業では、設計・計画・レビュー・ADRの承認に `start-work` の `references/decision-delegation.md` を適用する。本書のユーザー確認も有効な分担に照らし、同じ判断の再承認を求めない。記録・妥当性検証・個別の操作承認は維持する（ADR-0133）。
