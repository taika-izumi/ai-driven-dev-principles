...
**執行点**: 拡張 spec / 拡張 ADR をコミットする直前に点検ブロックの存在を確認する。無ければコミットしない。新設を含む拡張では、「全シナリオ共通: 新設の評価可能性」の執行点も同時に確認する。**再点検**: 点検後に規範の文言・適用範囲を追加・変更した場合は、当該差分について点検を再実施し、点検ブロックを更新する。
