...
| **R3** | 出所リスト行は `- <識別子>: <説明>` のコロン形に統一する。複数の識別子は `/` または `〜` で連結し、その直後にコロンを置く（下記「R3 の書き方」を参照）。**識別子はリストマーカーの直後に置く** | 括弧形の手順文と機械的に区別する。`- **関連**: ADR-NNNN` のように前に語を置くと出所リスト行と判定されず R2 違反になる（実測。慣用形なので注意） |
| **R4** | コードフェンス内に出所識別子を書かない | 配布先ファイルへ複写される書式テンプレートも、読まれるだけの例示データも対象 |
| **R5** | 書式例・サンプル値はプレースホルダまたは中立名（上表の許可リスト `X` / `Proj` / `<project>` に限る）で書く。**対象は識別子 5 種に限らず、実在のプロジェクト名・実在の絶対パス・実在の文書名を含む** | 実在を指す識別子を例示に使わない。識別子以外の実在の固有名も配布先では解決できない |
