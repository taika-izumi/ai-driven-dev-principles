...
| 4 | プロジェクト名部が中立名の許可リスト（`X` / `Proj` / `<project>`）のいずれかである |
...
| **R3** | 出所リスト行は `- <識別子>: <説明>` のコロン形に統一する。複数の識別子は `/` または `〜` で連結し、その直後にコロンを置く（下記「R3 の書き方」を参照） | 括弧形の手順文と機械的に区別する |
| **R4** | コードフェンス内に出所識別子を書かない | 配布先ファイルへ複写される書式テンプレートも、読まれるだけの例示データも対象 |
| **R5** | 書式例・サンプル値はプレースホルダまたは中立名で書く。**対象は識別子 5 種に限らず、実在のプロジェクト名・実在の絶対パス・実在の文書名を含む** | 実在を指す識別子を例示に使わない。識別子以外の実在の固有名も配布先では解決できない |

#### R3 が必要な理由（実例）

`skills/retrospective/SKILL.md:53` の次の行は、行頭の形が出所リスト行と同じだが中身は手順文である。「識別子で始まるリスト行を削除する」規則だとこの手順が静かに削除される。

```
- ADR-0006（意思決定の継続検出ルール）は本スキル中も常時適用される。ただし retrospective 内では対策の採否を決めないため、retrospective 起因の ADR 起票は発生しない
```

R3 によって出所リスト行をコロン形へ固定すると、括弧形で始まる行は手順文と判定でき、生成器はそれを R3 違反として停止できる。
...
区切りは**半角コロン＋半角スペース 1 個**に統一する（全角コロン `：` やスペースなし `:説明` は判定を通過してしまう）。**出所リスト行は出所の索引であり、規範文・手順文を書く場所ではない**（配布物から行ごと削除されるため、上記「R3 が必要な理由」の実例のように手順文を書くと配布先で消える）。

### 機械判定が届かない領域

R1〜R4 は識別子の位置の規約なので生成器が判定できる。**R5 は位置規約ではないため判定できない。** また、スクリプトの docstring と利用者へ表示するメッセージは判定の対象外である（コメント行のみを見るため）。この 2 つの領域は目視で確認する（検査の自動化は将来課題。Issue-0070／ADR-0084）。

**何を探すか**:

- **コードフェンス内のサンプル値**: 実在のプロジェクト名・実在の絶対パス・実在の文書名（識別子 5 種に当たらないため生成器は素通しする）
...
- **スクリプトの docstring・利用者へ表示するメッセージ**: 出所識別子（`#` で始まるコメント行は判定されるが、docstring と文字列リテラルは判定されない）

```
違反: print("走査を中止する（ADR-0054: silent tolerance はしない）")   ← 配布先で実行時に表示される
適合: print("走査を中止する（検出した違反は見逃さない）")
```

2026-08-08 の実測では、この目視の 1 巡目で 1 件を拾い 1 件を取り逃がした。取り逃としは配布物を通読するレビューで検出された。**目視は 1 回で終わらせず、生成後の配布物を読む工程を別に置くこと。**

**執行点**: 配布対象ソースを変更したら、コミット前に生成器を実行する。`skills/` を変更したなら `scripts/build-dist.ps1`、それ以外（`template.manifest` 記載ファイル・空インデックス生成対象）を変更したなら `scripts/sync-template.ps1`（両方を変更したなら両方）。規約違反があれば非ゼロ終了するため、修正しない限りコミットできる状態にならない。あわせて**両者を `-Check` で実行**し、生成漏れが無いことを確認する（`-Check` はどちらの側の陳腐化も検出するため、変更範囲によらず両方を回す）。
...
- 配布対象ソースの記法規約に適合しているか（「全シナリオ共通: 配布対象ソースの記法規約」/ ADR-0083。`docs/overview/principles.md` は配布対象なので `scripts/sync-template.ps1` が違反 0 件で通ることで確認する）
