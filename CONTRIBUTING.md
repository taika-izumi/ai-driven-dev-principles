...
**規約に適合していても配布物が壊れる型がある。** 生成器が判定できるのは識別子の位置（R1〜R4）だけで、次の 5 つは判定を通過する。生成後の配布物（`dist/skills/` と `template/`）を読んで確かめること。

| 探すもの | なぜ判定されないか | 違反 → 適合 |
|---|---|---|
| **括弧内に識別子以外の語が同居した行**（R1-a） | R1 は位置しか見ない | `規範（ADR-0073 の型）を守る` → 除去後 `規範（の型）を守る`。`同型の規範（ADR-0073）` と書く |
...
**この 5 つは繰り返し取りこぼされている。** 2026-08-08 の実測では、書式例の固有名は目視の 1 巡目で 1 件を拾い 1 件を取り逃がし、自己参照は 3 度にわたって別の箇所（本文中・書式例中・スキルの説明文中）で見つかった。いずれも配布物を通読するレビューで検出されたものである。**目視は 1 回で終わらせず、生成後の配布物を読む工程を別に置くこと。**
...
手順: 全 ADR を「現役 / 部分修正あり（本体現役・状態変更なし）/ 実質置換（Superseded 化）/ 廃止（Deprecated 化）」で判定し、状態変更は `decision-log` の `references/status-updates.md`「ステータス変更」手順で記録する。境界例の判定はユーザーと確認しながら行う。
