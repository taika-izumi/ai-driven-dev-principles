...
**執行点**: 拡張 spec / 拡張 ADR をコミットする直前に点検ブロックの存在を確認する。無ければコミットしない。**再点検**: 点検後に規範の文言・適用範囲を追加・変更した場合は、当該差分について点検を再実施し、点検ブロックを更新する。新設を含む拡張では、「全シナリオ共通: 新設の評価可能性」の執行点も同時に確認する。
...
- 規範・手順・観点の新設を含む ADR の記載要件（比較・期待検出量・退役条件）は「全シナリオ共通: 新設の評価可能性」に従う（ADR-0102。退役条件について判断主体の移譲のみの記載を認めない点は、上記の実行可能性チェックの一般則に対する特則）。
