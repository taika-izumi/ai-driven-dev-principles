...
**執行点**: 拡張 spec / 拡張 ADR をコミットする直前に点検ブロックの存在を確認する。無ければコミットしない。新設を含む拡張では、「全シナリオ共通: 新設の評価可能性」の執行点も確認対象に加える（確認時点は同節による）。**再点検**: 点検後に規範の文言・適用範囲を追加・変更した場合は、当該差分について点検を再実施し、点検ブロックを更新する。
    ...
    | 規範の廃止条件 | 定義済み / 不要（理由） | <廃止の条件、または判断主体の移譲先> |
...
- 新設する場合は、期待検出量（発火機会の見込みと捕捉したい欠陥型。発火機会は対象期間・計数手段を添えた計数可能な形で書く）と観測可能な廃止の条件を ADR に 1〜2 行で明記すること。廃止の条件には成否の向き（何が観測されたら廃止側か）まで書き、判断主体の移譲のみの記載は認めない（判断主体の移譲を許容する一般則〈ADR-0032〉に対する特則）
- 廃止の条件の再掲は、観測可能な条件と向きを書いた同一 ADR 内の過剰適合点検ブロックの当該欄の記載がある場合に限り省いてよい（観測対象の名指しのみ・向きの無い記載では省略の要件を満たさない。点検ブロックを spec 側に記録する拡張では、上記の 1〜2 行を ADR に直接書く）
...
- 規範・手順・観点の新設を含む ADR の記載要件（比較・期待検出量・廃止の条件）は「全シナリオ共通: 新設の評価可能性」に従う（ADR-0102。廃止の条件について判断主体の移譲のみの記載を認めない点は、判定条件の実行可能性に関する一般則〈ADR-0032〉に対する特則）。
