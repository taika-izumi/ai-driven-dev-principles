...
4. プロジェクトの目的・方針の正本を特定する。既存のREADME・概要・ロードマップ等を再利用する場合は、プロジェクト固有のAGENTS.mdにパスと必要な節を記載する。未指定時の標準入口は `docs/overview/project-purpose.md`。正本がなければAIが既存合意から草案をまとめ、不足・矛盾・重要な解釈だけを利用者に確認する。目的本文はプロジェクト側で維持し、このリポジトリの目的をコピーしない。

start-workは開始・再開時に正本を確認し、指定先や標準入口で確認できなければ「目的・方針の正本の整備」をそのセッションの作業候補に含める。前回見送った場合も次回は再提示するが、整備への着手を強制しない。指定先が読めない場合は、参照先の確認・復旧を含む候補として示す。
...
## 目的・方針の参照機能を既存プロジェクトへ更新する

プラグイン更新はスキルを届けるが、利用側のAGENTS.mdの共通指示は別に統合する。更新を行う利用者または担当AIは、次の作業を行う。

1. 既存AGENTS.md、固有指示、目的正本の指定と参照先を確認し、変更前の内容をGitまたは退避コピーで保全する。
2. 新しい共通の「作業の起点」「コンテキスト管理」の該当文だけを差分照合して統合する。AGENTS.md全体や目的本文をテンプレートで上書きしない。共通部と固有部の区別・意味が衝突する箇所は報告して判断を求める。
3. 固有の目的指定・本文・指示が残り、開始・再開の手順から正本の必要箇所を読めることを確認する。スキル更新のみ、共通部未統合、到達未確認をすべて導入完了と報告しない。

目的・方針の内容は各プロジェクトで変更できる。確定した変更は正本と決定記録へ反映し、途中なら未反映の内容と根拠を引き継ぐ。目的取得・read-onlyでの扱い・主担当と子の分担はstart-workの参照手順に従う。このガイドライン整備プロジェクトの目的は [目的・判断方針](docs/overview/project-purpose.md) を参照する。
