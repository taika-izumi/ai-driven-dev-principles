...
> **新設の評価可能性**: ガイドライン配信元リポジトリでは、規範・手順・観点の新設を含む ADR に、`CONTRIBUTING.md`「全シナリオ共通: 新設の評価可能性」の記載要件が適用される（要件の内容は同節を参照する。退役条件について判断主体の移譲のみの記載を認めない点は、前項の一般則に対する特則）。当該節が見つからない環境では本注記は適用しない。
