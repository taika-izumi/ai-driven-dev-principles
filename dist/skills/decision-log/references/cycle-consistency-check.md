...
1. **仕様のスナップショット性**: サイクル中の実作業・レビュー由来の変更が上流仕様に反映されているか。照合は上流仕様側の全体読解を伴う
2. **規範の書き戻し**: サイクル中に新設・改定され稼働している規範が、仕様と決定に記載されているか
3. **数値・完了基準の整合**: 件数・期待値が正本・下位仕様・実装の間で一致しているか。該当する数値が現れる全文書を同時に読んで照合する（1 文書の差分だけでは不一致を検出できないため）
...
**重複実施の抑止**: 同一サイクルで本検査を実施済み（確認の記録に `cyclecheck=` の行がある）かつ以後の差分が無い場合に限り、再実施を省略する。差分がある場合、観点 4（発動経路と記録順序の整合）は差分に関わる規範のみの再検査でよいが、観点 1（仕様のスナップショット性）・2（規範の書き戻し）・3（数値・完了基準の整合）・5（引用の整合）は照合対象文書の全体を読み直す。本検査以外のレビュー（独立レビュー・最終レビュー等）は同等検査とみなさない。昇格処理そのものが生む差分（ADR の Status 更新・決定インデックスの行更新・close する課題・handoff への記録）は「以後の差分」に数えない（検査→昇格→再検査のループに停止条件を与えるため）。
...
- 検査で ADR へ決定を追記した場合は、後続の粒度点検で通常どおり再照合する
- 新たな設計論点は課題として起票に留め、当該サイクルで対策設計に入らない

**記録**: 実施結果を handoff の「節目ごとの確認記録」の当該マイルストーン行へ `cyclecheck=` として併記する（行形式・命名規約は `session-handoff` スキル参照。Accepted 昇格処理を含むマイルストーンは名称に `Accepted 昇格` を含める）。検査のみを実施したマイルストーン（実装前昇格の後追い検査）の行にも書く。値は次の 4 つに限る:
...
**廃止**: 廃止の判断と時期はユーザーが判断し指示した時点とする。判断材料は cycle-reset で消えない残存記録（retrospective・worklog・課題・検査由来の修正コミット）に検出実績が現れていない事実とする。AI はその事実を棚卸しや retrospective の際に観測した場合、簡素化・廃止を候補としてユーザーへ提案する。
