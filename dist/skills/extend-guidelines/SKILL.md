...
配布する変更の実装・必要なレビュー・検証が済んだ時点で本手順へ戻り、`CONTRIBUTING.md`「全シナリオ共通: 配布対象ソースの記法規約」の「配布プラグインの版更新」と「執行点」に従って配布更新を実施する。版更新の条件・公開前後の追加修正の扱い・対象別の生成・検証・コミットは同文書を正本とし、ここには複写しない。
