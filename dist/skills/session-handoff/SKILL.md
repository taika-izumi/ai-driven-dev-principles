...
`start-work` の `references/project-purpose.md` の参照結果に従う。「関連ドキュメント」に所在・適用範囲・参照内容の根拠を、「進行中のタスク」または「既知のブロッカー・懸念」に未確定・未反映事項と影響を記載する。目的本文を毎回複写せず、正本未整備と目的未確定を区別する。前回参照版は履歴の識別に使い、次回の目的をその版へ固定しない。
