...
確定前レビュー（`pre-finalization-review`）の提示は、以下の 2 種類の確定点——spec 確定点（到達経路により (a)〜(c) の 3 通り）と plan 確定点——で**毎回**行う（発動の判断はユーザーに残す。発動はユーザーの指示のみであり、規模・不可逆性による自動必須化はしない）。以下は提示内の**推奨順位**の規則である。
...
スキル外で行う改訂（機械検証由来の修正・設計縮小の適用を含む）でも、改訂着手前に草稿を退避する（退避規定は `pre-finalization-review`「反復の実施」節に従う）。
...
**停止判定（決定 2）**: 反復の停止は件数・巡数でなく指摘の分類で判定する。「成果物の実体への新規の設計欠陥がゼロ、かつ残指摘が直前の修正に由来する機械検証可能な型のみ」となった巡を実質収束とみなす（第 1 経路）。実質収束は確定前工程の停止判定であり、実装時の独立レビューを代替しない（補完関係）。判断材料として、各巡の指摘を対象レイヤ（以下の 2 分類）で「設計骨格（決定の構造・条件・機構そのものの欠陥。判定条件・分岐・記録機構と、規範を発火点・記録先へ接続する配線の新設・変更・削除を含む——機構を減らす設計縮小も骨格に触れる改訂である）」と「周辺（文書間整合・写し漏れ・修正伝播・表現精度）」に振り分け、迷う場合（指摘・改訂のいずれの判定でも）は設計骨格側へ倒す（振り分けは提示に添える判断材料であり、記録・引き継ぎの機構は置かない）。
...
1. **ADR候補検出**: 実行中に行われた意思決定が `decision-log` の検出トリガー一覧に該当するか自己評価し、該当すれば `decision-log` スキルを呼ぶ（実行中に既に作成済みのものは除く。トリガーの定義は同スキルが正）。
   ...
   **設計を確定させた ADR ドラフトをコミットする場合は、コミットの直前が spec 確定点 (c) にあたる**（設計文書ファイルを作らない拡張の型）。Phase 2 の「確定前レビューの提示規則」に従って提示すること。
