...
確定済み実装計画の実行中に、実装時レビュー（独立レビュー）または実装者検証が「計画どおりでは欠陥が残る」型の指摘を出したときの、逸脱の型分類・採用基準・記録の正本。`start-work` の横断的ラッパー Pre・Phase 2 マッピング表、および `subagent-dispatch` の手順から参照される。
