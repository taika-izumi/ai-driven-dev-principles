...
2. **スコープで振り分け先決定**（上表）
3. **出所点検**（汎用スコープ、および本 repo（ai-driven-dev-principles）で実行し CLAUDE.md 追記へ振り分ける場合。本 repo の CLAUDE.md は template 経由で配布されるため）: 根拠エントリの出所プロジェクト数・モデル世代を数える。単一プロジェクトまたは単一モデル世代に偏る場合、CONTRIBUTING.md「全シナリオ共通: 過剰適合の点検」の是正パターン（適用例への降格 / 発動条件ゲート / 根拠と世代＋退役経路 / 見送り・スコープ降格）の適用をユーザーへ提示する
