...
> **本ファイルは現行の設計を表していない。** 配布メカニズム（`template/` 生成と プラグイン配布）の現時点のスナップショットは `docs/current/specs/2026-08-07-distributed-artifact-generation/` が正である。本ファイルは旧型式（単一ファイル）spec の 1 本として残置されており、扱いの方針は Issue-0008 が持つ。
