...
2. フォルダ外に置くべき資料が生じた時、または追記時のサイズ実測が目安値（デフォルト 10KB、1KB = 1000 バイト）を超えた時に、各分類フォルダ内で `NNNN-<slug>/` フォルダへの昇格を提案する（判断はユーザー。課題ファイルは同名のままフォルダ内へ移動し、README.md 化はしない。詳細は課題管理定義 `docs/overview/issue-management.md` を参照）
