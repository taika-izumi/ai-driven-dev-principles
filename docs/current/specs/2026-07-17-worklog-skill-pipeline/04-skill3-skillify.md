...
- **出力**:
  - スコープ別の成果物（プラグイン配信スキル／プロジェクトローカルスキル（利用ツールのスキル配置先。Claude Code は `.claude/skills/`、Codex は `.agents/skills/` など）／AGENTS.md 追記）
...
2. **スコープ3分岐（振り分け）**:
   ...
   - **プロジェクト固有だが価値あり** → そのプロジェクトのローカルスキル（利用ツールのスキル配置先。Claude Code は `.claude/skills/`、Codex は `.agents/skills/` など）
   - **固有ルールでスキル化不要** → そのプロジェクトの AGENTS.md
3. **ガード発火**: 「汎用パス かつ 本 repo でない」場合のみ警告＋ユーザー確認（①この場のプロジェクトローカルスキルとして作成 ②配信元 repo へ移動して実行 ③中止）。固有パスはガード不要でそのまま
4. **出所点検**: 汎用スコープ、および本 repo で実行し AGENTS.md 追記へ振り分ける場合、根拠エントリの出所プロジェクト数・モデル世代を数え、単一出所に偏る場合は CONTRIBUTING.md「全シナリオ共通: 過剰適合の点検」の是正パターンの適用をユーザーへ提示（ADR-0079）
