...
4. 再生成した内容に残存参照番号（`Get-ProvenanceLeak`）が 1 件も無い（`dist/` とソースの双方が同じ漏れを含む場合に、照合だけでは `Up to date.` になってしまうのを防ぐ。CI が `-Check` のみを回す運用でも「配布物に実在を指す出所識別子 0 件」を担保する）
...
- **本ブロックが塞ぐのはディスク上の生成物の陳腐化のみ。** セッションへ供給されるスキル本文の陳腐化は別の失敗様式であり、`/plugin marketplace update` の実行と本文の照合に依存する（Issue-0044）
