...
`-Check` の一致判定は次の 3 条件をすべて満たすこととする。
...
識別子を 1 つも含まない行はステップ 1 で「対象外」となり、変換の対象にもならない。

`.py` / `.ps1` ファイルはコメント行のみを上記に掛ける。

### 4. 変換

**変換は判定が「除去対象」となった行にのみ適用する。** 識別子を 1 つも含まない行は一切書き換えない（表の桁揃え、行末の強制改行、パス末尾のスラッシュなどを壊さないため）。

| 対象 | 処理 |
|---|---|
| 出所リスト行 | 行ごと削除する。削除の結果その節が見出しだけになれば、見出し行と隣接する空行も削除する |
| `（出所: …）` | 括弧を中身ごと削除する |
| バッククォートで囲まれた識別子 | 囲みごと除去する。識別子だけを外すと空のバッククォート対が残り、後段では除去できない |
| 括弧内の識別子 | トークンと隣接する区切り（`。`／`、`／`・`／`/`／`〜`／空白／`項目N`／桁のみの継続形 `/NNNN`）とともに除去する。区切りが連続する場合は繰り返し除去する。括弧の中身が空になれば括弧ごと削除する |
| コードフェンス内 | 変換しない（R4 により識別子は存在しない）。空行の畳み込みも適用しない（コード例の空行を保つ） |
...
`plugin.json` も書き込み前の規約判定の対象に含める（変換はせず複写のみだが、識別子が混入した場合に `dist/` を書き換える前に停止させるため）。ただし種別 5（`（出所[:：]…）`）は規約上「除去対象」であって違反ではないため判定ゲートを通過し、複写後の自己検査（`Get-ProvenanceLeak`）で捕捉される。欠損している場合は例外スタックではなく `[build-dist]` 接頭辞の診断を出して非ゼロ終了する。
