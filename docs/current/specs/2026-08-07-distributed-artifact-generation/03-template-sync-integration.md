# 03: template 同期への変換組み込み

## 対象ファイル

- `scripts/sync-template.ps1` — 判定・変換ステップと `-Check` モードを追加する
- `template.manifest` — `AGENTS.md` を同期対象に追加した（Layer 2 の内容正本が `AGENTS.md` へ移り、`CLAUDE.md` は `@AGENTS.md` インポートのポインタ 1 行になったため。ADR-0111）

## 責務

`template/` の生成に、ブロック 02 と同一の判定と変換を適用する。テンプレート配布物は配布先プロジェクトの**実ファイル**になるため、出所識別子の混入はプラグイン配布より直接的な害を持つ。

## インターフェース

```
scripts/sync-template.ps1 [-Check]
```

| モード | 動作 | 終了コード |
|---|---|---|
| 既定 | 規約を判定し、適合していれば `template/` を再生成する | 0 = 生成成功 / 1 = 規約違反（`template/` は変更しない） |
| `-Check` | 規約を判定し、ソースから再生成した内容と既存 `template/` を比較する。書き込みはしない | 0 = 一致 / 1 = 規約違反または不一致 |

`-Check` の一致判定はブロック 02 と同じ条件による（内訳は下記「姉妹生成器との一致点と意図的な差」を参照）。

## 動作

| ステップ | 処理 |
|---|---|
| 1 | `template.manifest` を読み、コメント行・空行を除いてファイル一覧を得る（現在 6 ファイル: `AGENTS.md` / `CLAUDE.md` / `docs/overview/principles.md` / `docs/overview/folder-structure.md` / `docs/overview/issue-management.md` / `docs/inbox/README.md`） |
| 2 | 空インデックス生成対象 3 ファイル（`docs/records/decisions/README.md` / `docs/records/retrospectives/README.md` / `docs/working/issues/README.md`）について、テーブルのデータ行と直後の引用ブロックを除去した内容をメモリ上で生成する |
| 3 | ステップ 1・2 で得た**全 9 ファイル分の内容**へ `Test-ProvenanceConvention` を適用する。違反が 1 件でもあれば違反箇所と規約 ID を出力して非ゼロ終了する |
| 4 | `template/` を完全削除する |
| 5 | 各内容へ `Remove-ProvenanceNotation` を適用し、`template/` 配下の対応するパスへ書き出す |
| 6 | `template/` 配下に実在を指す出所識別子が 1 件も残っていないことを確認する（判定と同じ適用範囲・同じプレースホルダ判別を用いる）。残っていれば非ゼロ終了する |
| 7 | `scripts/check-claude-md-size.ps1` を呼び、AGENTS.md の規模を計測する（警告のみ。同期はブロックしない） |

**判定（ステップ 3）を削除（ステップ 4）より先に行う。** 順序が逆だと、規約違反で停止したときに `template/` が消えたまま残る。

書き出し（ステップ 5）は `Copy-Item` によるバイト複製ではなく、テキスト書き出し（UTF-8 BOM なし・LF 固定）で行う。判定と変換を通した内容を書くため、複製では実現できない。

判定と変換のロジックは `scripts/lib/strip-provenance.ps1` を dot-source して共有し、`sync-template.ps1` 内へ複製しない。

## 姉妹生成器との一致点と意図的な差

`build-dist.ps1`（ブロック 02）と本スクリプトは同じライブラリを使う姉妹であり、次を**揃える**。片方だけに手当てが入っている状態は取りこぼしとみなす。

- 規約違反時は生成物に触れず非ゼロ終了する。診断は「位置と規約 ID」「違反行の内容」の 2 行
- 成功時も `Convention violations: 0` を出力する（判定が走った証跡を残すため）
- `-Check` の一致判定は `02-distribution-generator.md`「インターフェース」が定める 4 条件（内容一致／余分なファイルの不在／BOM の不在／再生成内容への残存する参照番号の検査）を同じく満たす。既存生成物の内容は比較前に LF 正規化する
- 書き出し後の自己検査は `Get-ProvenanceLeak` を呼ぶ。フェンス追跡や適用範囲判定を生成器側へ再実装しない
- ソース欠損・定義ファイル欠損は警告で通さず、生成物に触れる前に非ゼロ終了する
- 例外スタックを生で出さず、`[<生成器名>]` 接頭辞の診断を出す
- パイプラインから受けたコレクションは `@()` で囲む（1 件・0 件のとき `.Count` が使えなくなるため）

**意図的な差**は次の 2 点のみ。

| 観点 | `build-dist.ps1` | `sync-template.ps1` |
|---|---|---|
| `✓` 行の意味 | ファイル別の参照番号除去数。`-Check` でも表示する（陳腐化を直す前の下見になるため） | 書き出したファイル名。`-Check` では表示しない（書き込みを行わないため）。空インデックス生成対象には `(empty index generated)` を付す |
| 違反報告のパス表記 | ソースのパスと行番号 | 空インデックス生成対象は判定が空インデックス化**後**の内容に対して行われるため、パスへ `（空インデックス化後）` を付す（付さないと、報告された行番号がソースの別の行を指す） |

## 影響を受けるファイル

単位は出所識別子の**出現回数**である（`00-overview.md` と `05` は行数で数えているため、照合時は単位に注意する）。

| ソース | ソース側 | 空インデックス化後 | 生成物側 |
|---|---|---|---|
| `docs/overview/folder-structure.md` | 7 | （対象外） | 0 |
| `docs/overview/issue-management.md` | 3 | （対象外） | 0 |
| `docs/records/retrospectives/README.md` | 128（種別 1〜3 が 121、種別 4 が 7） | 12 | 0 |
| `docs/records/decisions/README.md` | 多数 | 0 | 0 |
| `docs/working/issues/README.md` | 多数 | 0 | 0 |
| `AGENTS.md` / `CLAUDE.md` / `docs/overview/principles.md` / `docs/inbox/README.md` | 0 | （対象外） | 0 |

`docs/records/retrospectives/README.md` のソース側 128 のうち 116 は一覧テーブルのデータ行にあり（種別 4 の 7 件はすべてここに含まれる）、空インデックス化で除去される。残る 12 が判定と変換の対象である。うち 1 行（21 行目）が R2 違反であり、ブロック 05 で先に是正する。

`docs/overview/folder-structure.md` にはこのほかプレースホルダが 7 箇所あるが、4 桁の数字を持たないため参照番号ではなく、判定・変換いずれの対象にもならない。

## 順序上の制約

**ブロック 05（ソースの規約適合化）が先行しない限り、本ブロックの変更は `sync-template.ps1` を失敗させる。** 実装順序は 05 → 03 とする。

## ADR-0027 との関係

ADR-0027 の実現方法は、同期が内容を変えないことを前提にした記述を 3 箇所持つ——「説明文は保持し、インデックス行のみ除去する」「`template.manifest` は verbatim コピー対象のみを列挙する」「`docs/inbox/README.md` は verbatim コピーする」。本ブロックは**同期の全対象**へ変換を適用するため、この 3 箇所すべての適用範囲が変わる（manifest 経路のファイルも verbatim ではなくなる）。ADR-0027 のファイルへ部分修正の注記を追加する（ステータスは Accepted のまま。`decision-log` の台帳監査における「部分修正あり（本体有効・状態変更なし）」の扱い）。これは `00-overview.md` の完了基準 7 で判定する。

シード基準（何をテンプレートに含めるか）と空インデックス生成そのものの規約は変わらない。

## このブロック固有の制約・前提

- `template/` は git 管理下の生成物であり、この点は変更しない
- 空インデックス生成ロジック（`New-EmptyIndexContent`）自体は変更しない。判定と変換はその出力に対して適用する
- `check-claude-md-size.ps1` の呼び出しは末尾のまま維持する（警告のみで同期をブロックしない性質も変えない）。計測対象は `AGENTS.md` である（`CLAUDE.md` はポインタ 1 行であり、測り続けると規範肥大監視が無音化する）

## 関連 ADR

- ADR-0027: テンプレート初期セットの基準を定義し、インデックス空生成を一般化する（本ブロックにより部分修正）
- ADR-0082: 保守者向けの根拠注記はソースに残し、注記を除去した生成物を配布する
- ADR-0033: 生成物は LF 固定で書き出す
