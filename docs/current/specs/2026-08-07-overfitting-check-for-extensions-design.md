...
- **関連 ADR**: ADR-0079（本設計の決定）。是正パターンの参照元: ADR-0032（観測可能な発動条件）/ ADR-0073（適用例への降格・撤回・根拠と世代・規範の廃止条件）。本設計が型を一般化する既存 ADR: ADR-0040（CLAUDE.md 事前判定。是正パターン②のゲート要求は同手順のゲート必須と同一）。部分修正: ADR-0099（観点 2〈システム種別依存性〉・3〈AI モデル/ツール依存性〉の引用の照合注記と雛形根拠欄の改定。2026-08-16）
...
- 過剰適合の芽が見つかった場合は、是正パターン（降格・ゲート・規範の廃止条件・見送り/撤回）で汎用性を回復する。特定状況の知見はゲート付きで残す価値があるが、汎用側に便益が無ければ規範化を見送る判断も是正に含む
...
4. **規範の廃止条件**: 追加する規範が不要になったことを判定する経路（エージェントが観測可能な条件、または「ユーザーが判断し指示した時点」への判断主体の明示的な移譲）が定義されているか
...
> 3. **出所点検**（汎用スコープ、および本 repo（ai-driven-dev-principles）で実行し CLAUDE.md 追記へ振り分ける場合。本 repo の CLAUDE.md は template 経由で配布されるため）: 根拠エントリの出所プロジェクト数・モデル世代を数える。単一プロジェクトまたは単一モデル世代に偏る場合、CONTRIBUTING.md「全シナリオ共通: 過剰適合の点検」の是正パターン（適用例への降格 / 発動条件ゲート / 根拠と世代＋規範の廃止条件 / 見送り・スコープ降格）の適用をユーザーへ提示する（ADR-0079）
...
| 規範の廃止条件 | 定義済み | 横断節「本点検自体の廃止規範」: 全拡張で問題なし/該当なしのみが長期継続し、かつ同型事象が中央ストアへ現れない場合、簡素化・廃止を候補としてユーザーへ提案（判断はユーザー） |
...
期待値はすべて等号（ちょうど N 件）で書く。下限方式は部分欠落を検出できない（確定前レビューの指摘）。参照件数の総数は実装計画の作成時に挿入全文から機械計数して確定し、計画の各タスクの編集内容と照合する（CLAUDE.md 検証規範）。
...
8. プラグイン更新後のスキル本文反映: ユーザーに `/plugin marketplace update ai-driven-dev-principles` を依頼し、改定スキルを起動して返る本文と repo 実ファイルを照合する（Issue-0044 の運用知見）
