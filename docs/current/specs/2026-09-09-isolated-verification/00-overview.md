...
- 状態: 2026-09-09、AIなしのコピー準備・プロセス管理・結果照合を先行実装。履歴提供（ADR-0150）は独立レビューと指摘対応・部品検証まで完了。通信条件と実エージェント経路は未検証・未達を含むため、通常利用は未対応。
...
以下は共通CLI完成時の契約である。現在利用できる先行部品と検証方法は`scripts/verification/README.md`、進行範囲はADR-0149を参照する。共通CLIと制限付きCodexの起動関数は未実装。
...
2. 一意の実行番号で新しい作業領域を作る。対象の未コミット変更と選択した未追跡ファイルを複製し、一覧とハッシュを保存する。全参照と入力worktreeのHEADから到達できるGit履歴を独立した管理領域へ移し、資料と履歴を子が探索できるようにする。本文の転記にはAIを使わない。
...
- [ADR-0150](../../../records/decisions/0150-provide-repository-history-to-verification-agents.md): スクリプトによる資料とGit履歴の提供。履歴提供の要求はユーザー承認済み、先行部品の実装とレビューを経て確定。
...
| V2 | 未コミット変更・対象の未追跡ファイル・空白や日本語を含むパスとGit履歴を複製し、ファイル・HEAD・参照先の途中変更があるコピーを採用しない。過去の本文・タグ・別ブランチを参照でき、Gitの探索先はコピー自身を指す |
