...
ローカルsbxを候補実装先とする。提案用と再実行用は別の実行設定を持つ。同じ製品であっても保護条件を共用済みにしない。再実行側の通信・追加ホスト経路の拒否が実証できなければ、基盤変更を相談してblockedとする。
...
共有workspace・共有skills・追加MCP・ホストDocker・ホストホーム・原本・controlへの経路は与えない。クリップボード書込、SSH転送等、通常のファイル共有以外のホスト作用も起動前に拒否を実証する。単に未使用・設定項目不明で合格にしない。解決不能なら自由なモデル作業は解放しない。秘密値・OSの資格情報ストアの内容を記録や子の環境変数へコピーしない。
...
| V2 | 子から原本・baseline・control・資格情報・追加ホスト経路へ到達できないこと。未確認時の起動拒否 |
