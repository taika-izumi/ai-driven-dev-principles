...
- **部分修正（2026-08-08・ADR-0083）**: 本 ADR の「説明文は保持し、インデックス行のみ除去する」は、出所識別子については適用されなくなった。`sync-template.ps1` は保持した説明文からも出所識別子を除去する（配布先で解決できない参照を配布物へ残さないため）。**空インデックス生成そのものの規約は変わらない**。除去は空インデックス化の後段で行われるため、判定・変換の対象は空インデックス化後の内容である
