...
- **ライフサイクル**（D15）: `id` は人間可読で安定な `<project>-<date>-<NN>`（NN＝そのプロジェクトのその日の連番、skill1 が既存 log 末尾を見て採番）。`processed.jsonl` の `outcome` は `skillified` / `rejected` / `merged` / `deferred` の4種。スキル2は開始時に台帳を読み処理済み id を分析対象から除外する（分析コストは未処理エントリ数でスケール）。`deferred` はその時点のクラスタ根拠数を記録し、新しい該当エントリが増えたときのみ再提示する。再提案防止は二層＝①台帳（厳密・処理済みは丸ごと除外）②既存スキル重複排除（あいまい・未処理でも既存 description 一致で除外）
...
- **コスト・留意**: エントリ側 `outcome`（作業結果 success/partial/failed）と台帳側 `outcome`（採否結果 skillified/rejected/merged/deferred）の2つの同名フィールドが並存するため、スキル doc に混同注意を明記する。id 採番のため skill1 が既存 log 末尾を読む必要がある
