...
- **部分修正（ADR-0132）**: 版更新義務・正本2定義・既定patch・生成物を直接編集しない原則は維持。更新時点と公開前後の追加修正を具体化し、実行手順の正本をCONTRIBUTING.mdへ接続する。反映手順2・3の利用側更新・供給本文確認は、利用環境への反映まで依頼された場合の手順として保持し、すべての配布更新の完了条件には含めない。StatusはAcceptedを維持する。
