...
設定機構の注意点は4つある。エージェント定義ファイルは起動時に読み込まれ、実行中のセッションで新規作成しても認識されない（上位ディレクトリの定義は発見される）。`--agents` のJSONは `permissionMode` を受け付けるが `mcpServers` は受け付けない（`Invalid input`）。`--permission-mode auto` は `--print` 実行に反映されず、開始イベントは `default` だった。定義ファイルの `mcpServers` は `--strict-mcp-config` と併用できず、併用時は接続が解決できないとして子の起動が拒否される。

定義ファイル側の制限も同日に実測した。拒否リスト（`disallowedTools`）でシェル・ファイル編集・ファイル作成・再委譲を外した子は、5ツールすべてを実際に呼び出して `No such tool available` になり、許可された読み取りだけが通った。接続の限定公開（`mcpServers`）は、`--strict-mcp-config` を外した実行で成立し、親のツール一覧に接続が現れないまま子だけが固定検査を実行できた（正常1成功・変異1失敗・復元1成功、保護代用品4件の拒否）。設定済みの他の接続は起動引数の拒否リストにサーバー名を並べることで、親子ともツール一覧から外せた。

親の権限チェックを無効にした起動（`bypassPermissions`）でも同日に測った。ツールを `Read` だけにした子は書き込み系ツールを持たず、保護代用品は無変更だった。`Write` を渡した子は保護代用品を上書きでき、`acceptEdits` では拒否された作業ディレクトリ外への新規作成も成功した。つまりこの起動では作業ディレクトリの境界も保護にならず、保護対象を外へ置く回避策は成立しない。書き込み系ツールを外す方法だけが残る。なお `Write` の1回目は「先に読んでいないファイルには書けない」として弾かれ、読んだ後の2回目で通った。これは誤爆を1段抑えるが保護ではない。

未確認は、対話セッション（autoモード）での実挙動、別OS・別版である。今回の成功をこれらへ読み替えない。
