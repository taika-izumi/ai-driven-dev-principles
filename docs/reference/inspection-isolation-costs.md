...
対話セッション（autoモード）での実挙動は翌日に実測した（下記）。残る未確認は別OS・別版である。今回の成功をこれらへ読み替えない。

### 対話セッションでの追加実測（2026-09-09）

出所: Issue-0136・ADR-0144、2026-09-09。同じ Claude Code 2.1.263・Windows 11 を、`--print` ではなく対話セッション（autoモード）で測った。[実行記録](../records/experiments/2026-09-09-claude-native-subagent-interactive.json)

子を起動する前に3点が観測できた。起動時に読み込まれたエージェント定義は対話セッションの担当一覧に現れ、ツール制限も反映される。定義ファイル内で宣言した接続は親のツール一覧に現れないまま子だけに渡る（`--print` 実行と同じ形）。そして Agent ツールの引数には子のツール許可リストを指定する項目が無く、指定できるのは担当の種別・モデル・隔離のみである。つまり対話セッションで測れる構成は、起動時に読み込まれた定義ファイルと内蔵の担当種別に限られ、新しい制限構成を試すには新しいセッションが必要になる。

許可リスト方式は対話セッションでも成立した。`tools: Read` の子は Bash・PowerShell・Write・Edit・Agent を実際に呼び出しても `No such tool available` で拒否され、許可された読み取りと結果回収は成立し、保護代用品は無変更だった（委譲元のハッシュ照合で確認）。同じ子に、遅延ツールの取得手段（ToolSearch）と NotebookEdit を実際に呼び出させても同じく拒否された。許可リストに載っていないツールを子が後から取得する経路は確認されていない。

一方、拒否リスト方式は保護手段として成立しなかった。`disallowedTools` でシェル・ファイル編集・ファイル作成・再委譲を外した子は、その5ツールこそ拒否されたが、残っていたツールを全数列挙させると、すぐ呼べるもの6件と遅延ツール14件があり、そのうち Artifact・NotebookEdit・SendMessage は書き換え能力を持ち、EnterWorktree・ExitWorktree・TaskStop は状態を変える。実際に NotebookEdit を使わせたところ、保護対象として置いた代用ノートブックを**承認要求なしで上書きした**。上書きはハッシュ不一致とセル内容の読み直しで確認し、証拠を複製したうえで復元した（基準値と一致）。拒否リスト方式は列挙した分しか外さないため、列挙漏れがそのまま書き込み経路になる。これは対話セッション固有の差ではなく、前日の測定が5ツールだけを試したことによる列挙の不完全性である（`--print` 実行での残存集合は未測定）。

測定方法の注意点として、子は最初「一覧に無いので呼べない」と自己判断して呼び出しを1度も発行せず、自己申告だけを返した。実際に発行するよう指示し直してはじめて拒否エラーが得られた。別の子は「対象ファイルは改変できない」と自己申告しながら、実際に呼ばせると上書きできた。自己申告と実呼び出しは証拠の等級が異なるため、制限の確認は実呼び出しと委譲元の状態照合で行う。

この時点の未確認は、`--print` 実行での拒否リスト方式の残存ツール集合、別OS・別版、Artifact と SendMessage の実効性、状態を変える残存ツール3件の実効性、固定検査の接続を子だけに渡す構成の対話セッションでの実行であった。最後の1件は下記で解消した。

### 固定検査の対話セッション実行（2026-09-14）

出所: Issue-0136、2026-09-14。Claude Code 2.1.270・Windows 11 の対話セッション（autoモード）で、定義ファイル側に接続を宣言し固定検査1操作だけを許可リストに載せた子（`tools: mcp__inspection__run_inspection`）を起動した。親は claude-fable-5-1、子は定義の指定で haiku、固定検査本体は Codex 0.153.4 のサンドボックス内で実行した。[実行記録](../records/experiments/2026-09-14-claude-native-subagent-interactive-fixed-inspection.json)

結果は `--print` 実行と同じで、Pester の正常1成功・変異1失敗・復元1成功、保護代用品4件（原本・共有ツール・セッションログ・接続プログラム）への書き込みは検査プロセスと子プロセスの両方で `UnauthorizedAccessException`、許可先への書き込みだけが通った。委譲元の照合では、試験領域の保護対象9件はハッシュ一致、追加されたのは検査用の一時ディレクトリ1件だけで、結果ファイルの上書きは事前の複製で保全した。追跡ファイルの変更はない。子に Write と Bash を実際に呼び出させると、いずれも `No such tool available` で拒否された。版の差（2.1.263→2.1.270）と親モデルの差で結果は変わらなかった。

測定方法の注意点は前回と同じで、子は初回「ツール定義に存在しない」と自己申告して呼び出しを発行せず、再指示ではじめて実呼び出しの拒否が得られた。

残る未確認は、別OS、Artifact と SendMessage の実効性（外向きで取り消せないため試行しない）、`--print` 実行での拒否リスト方式の残存ツール集合である。拒否リスト方式の子に残る状態変更系ツール3件（EnterWorktree・ExitWorktree・TaskStop）は、拒否リスト方式が保護手段から外れているため測定せず、規範上の影響なしとして閉じた（2026-09-14 のユーザー判断）。
