...
| 成果物を外へ取り出せる | `sbx cp VM:PATH ホスト` で回収できる。現在の実装は、固定エクスポーターが返すJSONデータ列を検査して回収し、子のtarをホストで展開しない。Codex側の候補比較/recheckで実証した | 実装・Codex側統合を実測済み（2026-09-17） |
| 通信の拒否 | 作成時の `--deny-network '*'` で、HTTP プロキシは 403、任意ポートへの接続は policy log に拒否として記録される。提案用では当該VMのキットallowと他11ホストdenyを照合し、2ホストの443番だけを許可する構成も実測した | 拒否/限定許可を実測済み（2026-09-14、09-17） |
...
## Codex収録済みテンプレートとモデル往復（2026-09-17）

- agent=codexでも、明示的にshell版の固定テンプレートを渡すとCodex本体は追加されない。Issue-0151で検出し、提案側を公式codex版の固定digestへ変更した。テンプレートの意味は、OS・実行ファイルを含むVM用の実行環境である。
- 新codex版はCodex CLI 0.149.1、実行パス/usr/local/share/npm-global/bin/codex。標準入力の転送はローカルsbx 0.42.1ではexecの-i有無の両方で成立した。cloudや別版への保証ではない。
- 2ホスト443番限定、OAuthセンチネルの5条件、workspace-write/neverの非対話起動、提案→通信なしVMでの候補比較→主担当採用→recheckを実測した。モデル出力だけで採否を決めず、外側の結果を照合した。
- 能力証拠は2026-09-17-task9-codex-template-evidence.md、往復記録は2026-09-17-task9-codex-roundtrip.md。Claude Code主担当からの実証は未了。
...
- APIキー方式のモデル往復（今回のモデル実証はOAuthのセンチネル方式だけ）。
