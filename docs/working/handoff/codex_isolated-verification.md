...
- **Last Updated**: 2026-09-09 10:24 (Asia/Tokyo)
- **Status**: in_progress
...
- **現在の作業**: タスク0の起動・保護試験。
  ...
  - 残り: 追加調査でもelevated明示とcommand/execの両経路でTCP接続が成功。Windows側のフィルター診断か別構成の判断が必要。execの最小AI試験は未実装・未実行。タスク1へ進まない。
...
2. Windowsのパケットフィルター診断か別の隔離構成を判断する。実行ユーザーと遮断規則のSIDは一致している。詳細は実測ノートを参照。
