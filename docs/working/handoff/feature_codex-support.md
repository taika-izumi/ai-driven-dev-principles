...
- [x] **検証 6 の Claude Code 側: 通過**（2026-08-25。push 後に `/plugin marketplace update` を実行し、インストール版が `0.1.12`・`gitCommitSha` がマージコミット `a9bc086` へ更新されたことを実測。スナップショットの `CLAUDE.md` は 11 バイト・`AGENTS.md` あり。`.agents/` と `dist/.codex-plugin/` の追加による退行なし）
