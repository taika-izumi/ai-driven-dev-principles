...
- **Branch**: master
- **Last Updated**: 2026-09-11 07:12 (Asia/Tokyo)
- **Status**: paused
- **Current Phase**: 準備完了 / 本比較の実行指示待ち
...
- **現在の作業**: 準備完了。本比較は0回。最新のユーザー指示は「検証準備段階を完了させるところまで」であり、モデルは停止・起動許可falseを維持する。
...
- 残り: 次の本比較実行指示を受けたら、Astra日時の2実行から開始。成立を照合してからSol日時と両モデルの小機能へ。新たな環境整備・成功済み診断を繰り返さない。
...
- Task 4の準備は完了。本比較の実行指示を待つ。Task 2・3を重ねて作成しない。Issue-0141・0142・0139は未対処。Issue-0135は触る際のフォルダ昇格の提案対象。
...
1. 本handoffと `docs/records/experiments/2026-09-11-openai-comparison-preparation-complete.md`、試験ルートのcontrol/preflight.json・budget.jsonを読む。準備は完了、本比較は未実施。
2. 本比較の実行指示を受けた場合にだけ、準備完了記録の具体的な起動方法でAstra日時2条件から開始する。モデル能力や手順の便益を、起動確認だけで判断しない。
3. 原本・固定検査・他worktree・stash・未追跡資料を保全する。Claude・sbx整備へ戻らず、比較開始を妨げる新規差分だけ確認する。
## 重要な意思決定の履歴
