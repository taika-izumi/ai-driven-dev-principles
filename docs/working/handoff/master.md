...
1. [ ] **Issue-0009**（flow）: ADR ドラフトの起票タイミングと議論収束の緊張 — ADR-0006 の即時ルールは毎決定で発動するため手戻りが構造的に再発する。同じ記録プロセス規範である **Issue-0010（再発の記録方法未定義）と同一サイクルでの一括対策を推奨**
2. [ ] **Issue-0002**（system）: sync-template.ps1 の改行コード非決定性 — 深刻度は低いが template 同期のたびに差分ノイズを出し続ける（Issue-0004 対策サイクルでも template README 2件に改行のみ差分が再発、git restore で回避）。数行で即修可能、早く消すほど累積コスト減
...
- ユーザー環境では AskUserQuestion と同一ターンのテキストが表示されない（Claude Fable 5 で確認済み）。対処は CLAUDE.md「ユーザーへの質問と意思決定要求」に規範化済み（ADR-0029。質問の自己完結＋説明が必要な場合はテキストのみのターン＋番号付き選択肢）
