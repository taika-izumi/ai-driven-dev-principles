...
- 課題一覧（唯一のバックログ）: `docs/working/issues/README.md`（open 計 43 件。2026-08-16 実測: 前回 45 − close 2）
...
1. [ ] **Issue-0084**（flow・本命候補）: マージ方式 --no-ff の完了フロー未配線。**5 サイクル連続で手動適用**（直近 `598b279`）。配線価値が最も高い
2. [ ] **Issue-0089**（flow・新規）: 指定席（議事録・インシデント報告等）の命名・索引規約が未定義（Issue-0088 系の続き）
3. [ ] **Issue-0073**（flow）: 計画の検証期待値の陳腐化（累計 6 件観測。直近サイクルでは発生なし）。Issue-0056・0042 と同時対策の候補
4. [ ] **Issue-0072**（system）: 半角括弧の同一視。1 行修正で塞げることまで確認済み（軽量に畳む候補）
5. [ ] **Issue-0075**（flow）: 規範文書の実効性観点がレビューにない
6. [ ] **Issue-0090**（flow）: インデックス 3 種・plans 残置・worklog 台帳の抑制未検討（実害の観測が再検討条件）
7. [ ] **Issue-0070/0072 のペア**（system）: 機械判定の届かない散文 / **Issue-0071/0077 のペア**（system）: 配布物の置き場非対称
8. [ ] **Issue-0085**（flow）: 消化記録フィールド新設のチェックリスト無し / **Issue-0087**（flow・低優先）: 発火表の統廃合契機
9. [ ] **Issue-0091**（system・低優先）: 完了済み移行 spec の乖離（Issue-0008 の一般形と同時に扱える）
10. [ ] **Issue-0057**（flow）: サブエージェント報告識別子の検証 / **Issue-0020**（flow）: ステージ内容確認 / **Issue-0044**（flow）: スキル改定の同セッション反映 / **Issue-0045**（flow）: open 課題の実行可能性点検（open 43 件・棚卸し価値高）
...
- **plugin 0.1.6 の配布反映が未実施**: ユーザーによる `/plugin marketplace update ai-driven-dev-principles` が必要（ADR-0090）。前サイクル分の**課題管理定義 `docs/overview/issue-management.md` の既存配布先への手動コピー**も未実施のままなら合わせて
- **リモート同期**: 本サイクル分（マージ `598b279` まで）の push はセッション終了処理で実施予定（ユーザー指示済み）。以後の push も自動では行わないため、必要な区切りでユーザーが指示すること
...
2. **未完の後始末**: `/plugin marketplace update ai-driven-dev-principles`（0.1.6 反映）→ 反映後に改定スキル本文と repo 実ファイルの突合（Issue-0044 の運用知見）。前サイクル分の課題管理定義の配布先手動コピーが未了なら合わせて
3. **次サイクルの候補（着手はユーザー判断）**: 抽出課題は issues に起票済み（open 43 件）。本命候補は **Issue-0084**（5 サイクル連続の手動 --no-ff で配線価値が最上位）。Issue-0088 系の続きは 0089/0090。軽量なら **Issue-0072**
4. **留意点**:
