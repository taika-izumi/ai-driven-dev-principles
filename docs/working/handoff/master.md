...
- **リモート同期**: `52551b3` まで push 済み（0.1.14 は配布へ反映済み）。**ローカル master は 8 コミット ahead（`eed1b63`〜`96b9218` 未 push）。0.1.15 は push 後に配布へ反映**。各ツールのローカルキャッシュへの反映は利用側の更新コマンド実行が必要（README「スキルのバージョンアップ」参照。ユーザーが実行）
...
- **Codex に本プラグインを GitHub 経由で登録済み**（実運用状態。配布版は 0.1.14。0.1.15 は push 待ち）。取り消すなら `codex plugin remove` ＋ `codex plugin marketplace remove`
...
1. **最初に実行**: `start-work`（Phase 0 で本ハンドオフを read）。**ローカル master が 8 コミット ahead**（`96b9218` まで未 push）のため、必要なら先に `git push origin master`（push で配布側も 0.1.15 へ）
