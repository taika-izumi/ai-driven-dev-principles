# Handoff: Issue-0014 対策サイクル完了・次サイクル待機

- **Branch**: master
- **Last Updated**: 2026-07-05 (Asia/Tokyo)
- **Status**: ready-for-next-cycle
- **Current Phase**: Issue-0014 対策サイクル完了（merge: cc67f73、retrospective 実施済み）/ 次サイクル未着手

## 作業の目的・背景

本リポジトリ `taika-izumi/ai-driven-dev-principles` は、AIエージェント駆動開発のメタ・ガイドライン（5原則 + スキル群 + ADR）を整備するプロジェクト。

直近サイクルは「Issue-0014: .gitattributes がなく改行正規化が各自の core.autocrlf 設定任せ」の対策。`.gitattributes`（`* text=auto` のみ）を追加し `git add --renormalize .` を実行（index は既に全 LF のため差分ゼロ）、CRLF→LF 正規化を使い捨てファイルで実機検証して完了（ADR-0037, Accepted）。template への配布は ADR-0027 のシード基準に該当しないため見送り。小規模のため plan/spec は作らず ADR-0037 が設計記録を兼ねた。Issue-0014 は close。これにより Issue-0002 → ADR-0033 → Issue-0014 → ADR-0037 と続いた改行問題の課題系列が終息した。retrospective は新規課題なし（Issue-0016/0017 へ進展追記のみ、ADR-0031）。あわせて、前サイクルから継続観察だった `CLAUDE_CODE_DISABLE_MOUSE_CLICKS=1` の実機検証を本サイクル冒頭で完了した（環境変数 `1` を確認、AskUserQuestion を2回運用し誤確定なし。ADR-0036 の環境ベース2分岐規範の初運用）。

## 関連ドキュメント

- 課題一覧（唯一のバックログ）: `docs/working/issues/README.md`（open 6件 / closed 11件）
- 直近サイクルの ADR: ADR-0037（Accepted）
- 直近サイクルの retrospective: `docs/records/retrospectives/system/2026-07-05-gitattributes-eol-normalization.md`（新規課題なしのため flow/ ファイルなし）
- 課題管理の規約: `docs/overview/folder-structure.md` §7
- ADR インデックス: `docs/records/decisions/README.md`（0001〜0037）
...
バックログは `docs/working/issues/README.md` に一元化済み（open 6件）。Issue-0014 close を反映した目安:

1. [ ] **Issue-0006**（flow）: 横断変更の計画網羅漏れ / **Issue-0016**（flow）: ツール適用結果を独立確認せず既遂とみなすと記録と実体が乖離する / **Issue-0017**（flow）: 対策検討の初手に「環境・ツール設定による構造的解決の調査」を促す観点がない — 筆頭（検証・プロセス品質系）。0016 は対策方向（read-back verification をリスク比例・git 非依存で導入）がユーザー合意済みで、Issue-0014 サイクルでも実践例を追記済み。0017 は Issue-0014 サイクルが有効性の実例になった（検討状況に追記済み）。0016/0017 は「進め方のチェック観点」系で、まとめて扱える可能性がある
2. [ ] **Issue-0003**（system）: conversation_log.md の分類 / **Issue-0008**（system）: 旧型式 spec 8本の維持方針 — 2番手。どちらもユーザーの方針決めが主
3. [ ] **Issue-0015**（flow）: マルチライン文字列をツールのシェル種別に合わせる規範・チェックがない — **低優先**。実害は軽微（回避策 `git commit -F` は有効と確認済み。ただし規範・チェックは未整備）
4. [ ] **Theme C 問題A**「プロジェクト固有用語集（ユビキタス言語）の仕組み」: 大テーマ（課題ではなく作業テーマのため issues 対象外）。規模が大きく brainstorming からの本格サイクルが必要
5. [ ] **ADR-0013 / 0014 / 0018 の Proposed 据え置き解消**: 最後尾。どこかのサイクルの Post チェックで昇格/棚卸しの判断だけ済ませると台帳が締まる
...
4. **次に走らせる作業（候補）**: 「未着手のタスク」の優先順位付け（1: Issue-0006/0016/0017 → 2: Issue-0003/0008 → …）を参考にユーザーが選択（着手はユーザー判断。必ず次サイクルではない。抽出済み課題は全件 issues 起票済みで、直近サイクルの新規起票はなし）
5. **留意点**:
