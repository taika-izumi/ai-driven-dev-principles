...
- 2026-08-31（ADR-0121 サイクル）: **再発 1 回**（claude-fable-5）。ADR-0121 ドラフトのコミット（`5fe9afc`）で、handoff 作成時に `git add` 済みだったステージ残留（初期版）が同乗した。`git add` は pathspec 指定だったが `git commit` はステージ全体を拾い、直前の `git status --short` で `AM` 表示を視認しながら「このコミットに入れるべきか」の判定工程が抜けた（2026-08-05 のスキル化サイクルの再発と同型。表示の視認と混入判定は別工程、の実測 2 例目）。実害なし（古い handoff 版が履歴に 1 コミット入ったのみ・最新版は後続コミットで確定）
