...
- 2026-09-08: MakeAiInstructionsのIssue-0136で、PowerShellの二重引用符内のバッククォート＋0によりIssue文書へNUL1件が混入し、子PowerShellの実行ファイル誤指定でも検査未実行が発生した。双方を修正・再検証済み。ユーザー承認によりC08（PowerShell・Pester固有の落とし穴）の再発材料として追記し、別Issueは起票しない。詳細はdocs/records/retrospectives/flow/2026-09-08-issue-0136-inspection-protection.mdの事例2。
