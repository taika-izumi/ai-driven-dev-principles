...
逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / Step 6-1 期待値のうち「本計画ファイル＝新規（未追跡）」は不成立（計画確定コミット 40def97 で既にコミット済みと git ls-files・git log で実測。期待値が計画のコミットより先に書かれたことによる陳腐化。他の期待値は全一致、本記録行の追記により計画ファイルは修正扱いでコミットへ同梱）
