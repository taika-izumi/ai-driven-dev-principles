...
期待: すべて `0`。`出所識別子`・`安定識別子`・`プロジェクト識別子` は残るので、`識別子` 単独の検査は否定後読みを使う（`プロジェクト識別子` は 4 件あり、上の Python は 4 を返す。4 件すべてが `プロジェクト識別子` であることを目視で確認して 0 とみなす。`store-format.md` 8 行の既存 1 件と、本タスクで裸の `識別子` から明示した 3 件〈`worklog-extract/SKILL.md` 17 行・`worklog-record/SKILL.md` 44 行・`store-format.md` 38 行〉の合計）。
...
逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / Step 2 の特例列挙に `フル巡` `差分確認巡` の扱いが無く表どおり `回` に置くと「フル回」になる、ADR-0123 の Consequences が「フル巡とフル実施は後者を据え置き前者を廃止」と定めているため `フル巡` 36 件を `フル実施`・`差分確認巡` 14 件を `差分再確認` とした、ADR 本文で委譲元が確認

逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / `pre-finalization-review/SKILL.md` 112 行の「構文・スコープ・足場の配線」を「足場の接続の記述」と訳したが、この文はコード上の局所的な誤り（構文・スコープ・足場の結線）の列挙であり文書間の相互参照ではないため「足場の接続」へ訂正した、Task 10 第 3 段で仕様書側との食い違いとして検出

逸脱記録: 機械的な適応 / 採用 / `巡` の置き換えで `pre-finalization-review/SKILL.md` の節見出し「観点と初回フル巡の体数」が変わるため見出しと引用 5 箇所を「観点と初回フル実施の体数」で統一した、委譲元の指示「初回のフル実施の体数」を見出しへ適用すると引用と食い違うため委譲先が実態優先で見出し側を採った、あわせて「前巡レビュアー」等の助詞なし複合語へ「の」を補った

逸脱記録: 機械的な適応 / 採用 / Task 6 は 239 箇所あり長時間の書き込みになるため委譲を 2 回（語「巡」125 件と残り 13 語 114 件）に分け中断時の復旧単位を作った、計画は 1 コミットだがコミットも 2 つに分かれる

逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / Step 3 の注記が `プロジェクト識別子` を 1 件と想定していたが、Step 2 の指定で裸の `識別子` 3 件を `プロジェクト識別子` へ明示するため作業後は 4 件になる、注記を 4 件へ訂正した、Task 14 の検証は `プロジェクト` も除外するパターンなので 0 で変わらない

逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / `op-finalize.md` 15 行を「現役性点検」を含む箇所として名指ししていたが実際の文言は「現役の申し送り・懸念」だった、置き換え表の一般規則（現役→有効）を当てて「有効な申し送り・懸念」とした、「現役性点検」は `op-cycle-reset.md` にのみ存在
...
実測値（2026-09-05）: **4 群の合計 点検 388 件・修正 3 件**。群 A 点検 171・修正 1 / 群 B 点検 29・修正 0 / 群 C 点検 28・修正 0 / 群 D 点検 160・修正 2

| 群 | 昇格 | 移設 | 剪定 | 裸の確定点 | 小計 | 修正 |
|---|---|---|---|---|---|---|
| A（`skills/`） | 61 | 40 | 3 | 67 | 171 | 1 |
| B（template 対象） | 21 | 8 | 0 | 0 | 29 | 0 |
| C（拡張手引きほか） | 9 | 17 | 0 | 2 | 28 | 0 |
| D（現用仕様書） | 37 | 87 | 15 | 21 | 160 | 2 |
| 合計 | 128 | 152 | 18 | 90 | 388 | 3 |


群 A の内訳: `昇格` 61 件（修正 0）・`移設` 40 件（修正 1 = `session-handoff/SKILL.md` の操作一覧の finalize 行を「記述の移設」へ）・`剪定` 3 件（修正 0）・裸の `確定点` 67 件（修正 0・種別の追加も 0）。修正 3 件はすべて `移設` で、`session-handoff` の finalize 手順を指す箇所を「記述の移設」へ揃えたもの（群 A: `session-handoff/SKILL.md` の操作一覧、群 D: `2026-08-13-handoff-bloat-control/00-overview.md` と `2026-08-06-handoff-pruning-and-status-design.md` の同手順の要約）。裸の `確定点` は 4 群で 90 件すべて据え置き（種別の追加も 0 件）。`昇格に至らない訂正`（`plan-deviation-defaults.md` 44・72 行）は同じ文に「確定分類（昇格後の型）」「型分類」があり何が昇格するのか読み取れるため据え置いた
...
逸脱記録: 実体に合わせる調整 / 採用 / 群 A は 171 件を点検して修正が 1 件だけだったため 1 コミットで実施した、長時間の書き込みで求められるコミット分割は復旧単位を作るためであり変更 1 行では意味を持たない、群 B から D は別の委譲で扱う
...
逸脱記録: 実体に合わせる調整 / 採用 / 対象が 1 語 1 ファイルのみでサブエージェントへの委譲コストが作業内容に見合わないため委譲元が直接実施した、他タスクの委譲方針（Task 1 から 12 を委譲）からの外れであり、検証（実測 1 件・置き換え後 0 件・BOM と CR なし）は委譲時と同じ手順で行った
...
逸脱記録: 実体に合わせる調整 / 採用 / 群 D は 280 箇所あり長時間の書き込みになるため委譲を 3 段（改名に揃える群 67 件・`識別子` 85 件・型 1 の残り 116 件）に分け復旧単位を作った、計画は 1 コミットだがコミットも複数に分かれる

逸脱記録: 実体に合わせる調整 / 採用 / `2026-08-28-start-work-responsibility-split-design.md` が `skills/start-work/SKILL.md` 16 行を逐語引用しており引用元の現行文言が「節目の前後で行う共通処理の接続の記述」に変わっていたため引用を揃えた、結果として群 D の `配線` が 26 件から 25 件へ減った、仕様書は今どうなっているかを書く文書なので引用元に揃えるのが正しい

逸脱記録: 実体に合わせる調整 / 採用 / `spec 確定点 (a)(b)(c)` を「spec 確定点」へ平坦化せず `pre-finalization-review` の定義表の型名（機能ブロック設計を経た場合、設計文書型、設計文書兼用 ADR 型）を割り当てた、平坦化すると仕様書が区別している 3 つの到達経路の情報が失われるため

逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / 「足場の配線」の訳が `skills/` 側と食い違ったため委譲元が `skills/pre-finalization-review/SKILL.md` 112 行を「足場の接続」へ訂正した、帰結の本文は Task 6 の逸脱記録へ記載

逸脱記録: 実体に合わせる調整 / 採用 / `2026-04-25-record-strengthening-design.md` 113 から 121 行の罫線ボックス内の語が長くなったため枠と各行のパディングを再調整した、調整後は 9 行すべて表示幅 65 で揃っていることを委譲元が検算した

逸脱記録: 実体に合わせる調整 / 採用 / `識別子` の複合語のうち委譲元が「の」挿入の例に挙げたのは `残存識別子自己検査` のみだったが同型の `残存識別子検査` 3 箇所も直訳すると読みにくいため「残存する参照番号の検査」へ揃えた、同ファイル内の据え置き表現「残存する出所識別子」に倣った、仕様書の記述内容と判定条件は不変

逸脱記録: 実体に合わせる調整 / 採用 / 「B 群（条件発火・6 条件 12 項目）」は `B 群`→`条件発火` の置き換えで「条件発火（条件発火・…）」の重言になるため括弧内の冗語を削り「条件発火（6 条件 12 項目）」とした、意味と件数は不変
...
逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / タイトルの走査パターンに `観点 ?[1-5]` を含めたため Issue-0066 が 29 件に数えられたが、置き換え表は「観点 1〜5 は名前を据え置き参照時に中身を添える」と定めており同課題のタイトルは無変更が正しい、したがって変更したのはタイトル 28 件と一覧 28 行の計 56 箇所で、設計書の 29 件・計 58 箇所には走査の副作用が 2 箇所含まれていた

逸脱記録: 実体に合わせる調整 / 採用 / `docs/records/decisions/README.md` が編集の際に全行 CRLF へ変わる事故が起きたが委譲先が検出して LF へ戻した、対象 3 ディレクトリ全体を再走査して CRLF と BOM の残存が無いことを委譲元も確認した

逸脱記録: 実体に合わせる調整 / 採用 / 置き換え後のタイトル 6 件に重言と不自然な語が生じたため委譲元が是正した、`条件発火の発火判定`→`条件発火の判定`、`接続の記述化`→`接続の記述への置き換え`、`2 層の接続の記述`→`2 層の接続`、`接続の記述で規範化`→`接続で規範化`、`接続の記述行`→`接続を書く行`、`節目の確認の実施の可視化`→`節目の確認の可視化`、いずれも一覧の該当行も同時に揃えた
...
逸脱記録: 実体に合わせる調整 / 採用 / 対象が 2 節・5 行・13 箇所のみでサブエージェントへの委譲コストが作業内容に見合わないため委譲元が直接実施した、検証は委譲時と同じ手順で行い変更 5 行がすべて対象 2 節の行範囲に収まることと節見出し `Post ラッパー消化記録` が据え置かれたことを機械判定で確認した
...
| # | 完了条件 | 判定 | 実測・所在 |
|---|---|---|---|
| 1 | 4 群で置き換え、各語の完了時に列挙外の変更が無いことを確認 | OK | 残存 0 件。各タスクで変更行が対象語のみで説明できることを機械判定した |
| 2 | 型 2 の 4 語を 4 群で目視点検（点検の実施を計画に記録） | OK | Task 7 の欄に記録。点検 388 件・修正 3 件 |
| 3 | 全 `skills/*/SKILL.md` が 20,000B 未満 | OK | 最大 `retrospective/SKILL.md` 19,051B |
| 4 | 表の新設・manifest 追加・`AGENTS.md` 1 行・`CONTRIBUTING.md` 新節・配置の早見表 1 行 | OK | 5 点すべて実体で確認 |
| 5 | 見出しや書式の名前の変更・`session-handoff` の汎用 1 文・他 5 スキル 7 箇所 | OK | Task 3〜5。汎用 1 文は `session-handoff/SKILL.md` 106 行 |
| 6 | 記録型文書のタイトルと一覧の記載 | OK | タイトル 28 件・一覧 28 行 = 56 箇所（走査ヒット 29 件のうち Issue-0066 は観点の名前のみで据え置き） |
| 7 | 執行点 4 手順を通過（両生成器の `-Check` が exit 0） | OK | Task 13。生成器実行・両 `-Check` exit 0・生成物の目視 2 回・生成物を同じコミットに含めた |
| 8 | Issue-0119 を closed・結論に ADR-0123・一覧を更新・close 時の移設判定 | OK | 移設判定は 3 分類すべて判定し結論節へ記録 |
| 9 | ADR-0123 を Accepted へ昇格（サイクル全体整合検査を実施） | OK | 検査 5 観点で指摘 6 件を反映してから昇格。粒度の点検でタイトルを拡張 |
| 10 | 総バイト数と増分率・`CONTRIBUTING.md` のサイズを ADR に記録 | OK | ADR の Consequences。skills +0.65%・CONTRIBUTING +3.19% |
| 11 | 本サイクルの成果物が新しい語彙で書かれている | OK（一部は次サイクル） | 設計書・ADR・本計画は「使用」0 件（言及のみ）。**ハンドオフは設計書 6-1 により本サイクルでは書き換えない**ため、この分は配布物更新後の次サイクルで満たす |
| 12 | version 0.1.19 | OK | `.claude-plugin/plugin.json`・`marketplace.json` |
