...
実測値（2026-09-05）: 群 A 点検 171 件・修正 1 件 / 群 B 点検 29・修正 __ / 群 C 点検 28・修正 __ / 群 D 点検 160・修正 __

群 A の内訳: `昇格` 61 件（修正 0）・`移設` 40 件（修正 1 = `session-handoff/SKILL.md` の操作一覧の finalize 行を「記述の移設」へ）・`剪定` 3 件（修正 0）・裸の `確定点` 67 件（修正 0・種別の追加も 0）。裸の `確定点` は据え置く方針どおり全件そのまま。`昇格に至らない訂正`（`plan-deviation-defaults.md` 44・72 行）は同じ文に「確定分類（昇格後の型）」「型分類」があり何が昇格するのか読み取れるため据え置いた
