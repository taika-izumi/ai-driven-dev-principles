...
- [x] 機構が特定できた場合だけ、対象VM・代用品・具体コマンド・停止・保全の実機試験案を作る。実機試験の操作は既存の固定smoke承認へ混ぜない。（2026-09-14: `docs/records/experiments/2026-09-14-v3-capability-test-methods.md` で取得元を特定し、ADR-0192の個別承認で新規VM1台の試験A〜Dを実施。SSH転送拒否・資源実効値・有限負荷中の外側停止・起動世代が成立。自動停止30秒とcp/execの自動起動を申し送り）
