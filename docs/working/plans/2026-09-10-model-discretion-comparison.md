...
- モデルは `gpt-6-astra / medium` と `gpt-5.6-sol / medium`（両モデルの親子実起動・mediumを確認済み）。既存契約枠のみ。
...
- [ ] Astraに必要な全項目pass後に `codex-bug-discretion` と `codex-bug-guideline` を順に起動する。片方が使った解法や差分をもう片方へ渡さない。30分期限と残る240分枠の双方を外側で計数する。
