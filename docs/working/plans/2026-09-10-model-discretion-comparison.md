...
- [ ] 通常ユーザーの認証、試験専用設定、書き込み境界、親子停止方法、モデル名と利用量の取得方法を、実際の実行ファイル・引数配列・許可パスとともに提示する。モデル起動を伴う事前確認と本比較の順序、各30分・作成側240分の上限とADR-0169の継続条件、モデル別の時間・利用量の見込み（仕様02）、利用者に必要な操作を含める。
