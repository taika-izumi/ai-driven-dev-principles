...
- [x] recheck 経路を先に実装する。`recheckArtifacts` が非空なら VM・モデルを起動せず、`origin=reused-tests`、`sandbox=null`、`stopState=not-created`、`artifacts` は前回hashのまま、`manifestPath` は `control/proposal/manifest.json` を新規作成して返す。
- [x] `proposal-export.py` を書く。VM内で `/home/agent/workspace/proposal/` 配下の `tests/test_*.py`・`tests/__init__.py`・`replacements/**/*.py` と `proposal.json`（summary、findings）を読み、`{"schemaVersion":3,"runId":...,"summary":...,"findings":[...],"files":[{"kind","path","contentBase64"}]}` を stdout へ1行で出す。自身は標準ライブラリだけを使い、上限を超えるファイルは含めずに `truncated` を立てる（外側はそれを拒否理由にする）。
- [x] 提案経路を実装する。`New-VerificationSandbox proposal` → `Copy-`/`Confirm-`（proposal-input を `/home/agent/workspace/source` へ）→ 依頼文（目的・合格条件・作業先 `/home/agent/workspace/proposal`・書式・禁止事項・残り時間〈proposalSeconds と全体期限から算出した秒数。助言であり強制は外側の上限で行う〉）を `startupArgv` の Codex へ stdin で渡す `Invoke-VerificationSandboxCommand`（proposalSeconds、`OutputSignature=$null`。CommandRecord は `transportVerified=null` で `quarantine/` に置き、ready 条件に使わない）→ `proposal-export.py` を cp して `python3` で実行（stdout 上限 maxWireBytes、署名は JSON 1行の parse 成功）→ 停止。子の実行イベントと最終応答は `quarantine/` に未信頼データとして保存する。
- [x] `Test-VerificationProposalEnvelope` を実装する。wire容量→重複キー・深さ・未知キー・型→runId→件数（maxProposalFiles）→path規則（UTF-8 POSIX相対、絶対・ドライブ・UNC・空成分・`.`・`..`・バックスラッシュ・コロン・NUL・末尾空白/ドット・Windows予約名、大小文字無視の重複、親子衝突、`.git`/`.codex`/`.claude`/`.agents`/`.mcp.json` 配下）→kind（test は `tests/` 接頭辞なしの相対名で `test_*.py`/`__init__.py`、replacement は baseline に存在する `.py` 通常ファイル）→base64正規性→復号後サイズ（maxFileBytes・maxProposalBytes）→UTF-8テキスト、の順に検査し、最初の違反で全体を拒否する。test 0件は拒否、replacement 0件は再現のみとして許す。
- [x] `accepted/tests/<path>`・`accepted/replacements/<path>` を CreateNew で生成し、祖先にリンク・再解析ポイント・既存ファイルがあれば拒否する。外側でサイズ・SHA256を計算して `control/proposal/manifest.json` に保存し、`manifestHash` を返す。`testsManifestHash`（testの相対パス・サイズ・SHA256をパス順に正規化したSHA256）も保存する。
- [x] 停止確認後に `status=ready`。停止未確認は受信済みでも `incomplete`（`stopState=unverified`）。時間超過は `timed_out`。`runtimeFailure` は捕捉して `sandbox` に確定済みhandleを残す。
- [x] `tests/ProposalV3.Tests.ps1` で、recheck が VM 0回、正常Envelope が ready、架空runId、リンク相当パス、`..`、予約名、大小文字重複、過大wire、復号後超過、途中切断（不完全JSON）、test 0件、replacement が baseline に無い、非.py、停止未確認 → incomplete、`quarantine` 保存を確認する。実Codex・実VMは起動しない。
- [x] コミットする。

逸脱記録: 実体に合わせる調整 / 採用 / proposal.schema.json に任意の truncated（真偽値、true は拒否）を置く（本計画のエクスポーターの打ち切り通知、受理される Envelope に仕様外の意味を加えない）、エクスポーターの固定 argv を python3 と /home/agent/proposal-exporter/proposal-export.py と runId・件数・容量上限の引数で固定し Codex の作業ディレクトリを /home/agent/workspace/source にする（タスク9で実機確認）、timed_out の優先は status と全体期限への到達で判定し reason で分岐しない、時間超過や非0終了では受信しても Envelope 検査と accepted 生成をしない（仕様02「失敗・時間超過で正常提案を確定しない」）、停止を受信直後で Envelope 検査の前に置く、path 規則に Windows の禁止文字と制御文字（0x00-0x1F・0x7F）を足し予約制御パス .verification-tests と .verification-control（仕様01の2名、完全一致）へ書かない、エクスポーターは上限超過に加え通常ファイルでない候補・リンクされたディレクトリ・tests や replacements がディレクトリでない場合も除外して truncated を立てる、重複キーと深さの検査を .NET の解析時検査で行う（2026-09-15 タスク4 実装者報告）
逸脱記録: 設計の変更 / 不採用 / エクスポーターを cp で搬入せず標準入力で渡す代案（搬入1回と子による改変の余地を減らす）、本計画は cp での搬入を指定し出力は外側で全面検査するため採用せず、既定の基準（検査の仕組みが無い・自分で持ち込んだ後退）のいずれにも当たらない（2026-09-15 タスク4 実装者報告）
逸脱記録: 実体に合わせる調整 / 採用 / 偽sbxの後片付け補助 Get-CaseFakeProcesses・Stop-CaseFakeProcesses を試験ファイルから FakeSbxScenario.psm1 へ移して共用（Issue-0147 の試験足場の制約をタスク4以降の試験で重複実装しないため、主担当が指示）
...
- [x] 合成題材を確認する。ホストに Python 3 があれば `pilot-source` で before（原文）が非0、after（replacement 適用）が 0 になることを1回実行して記録する（無ければ未確認と記し、タスク8で確認する）。ホストでの実行は固定の題材に限り、回収物には適用しない。
- [x] 入力検査を実装する。`ProposalResult.status=ready`、runId一致、`manifestHash` と `accepted` 現物の再照合、origin ごとの sandbox/stopState 条件、baseline の全ファイル manifest と期待hash の照合。不成立は blocked。
- [x] `replay-inputs/before` と `after` を外側で作る。before は baseline の作業ファイル（`.git` を除く）、after は同じファイルへ検査済み replacement を適用したもの。生成後に before と after の全ファイル差分を取り、(i) 差分のパス集合が replacement 一覧に**含まれる**こと、(ii) 各 replacement の after 側の内容が accepted のバイトと一致すること、を確認する（内容が基準版と同一の replacement は差分に現れなくてよい。不成立は incomplete）。両方の `.verification-tests/` に同じ tests を置き、各入力の manifest（path・size・sha256）と `testsManifestHash` を保存する。mode は replacement なしで `reproduction-only`、ありで `candidate-comparison`、recheck で `recheck`（現在版を after とし before=null。recheck では (i)(ii) の差分照合を適用せず、`testsManifestHash` の一致と baseline manifest の照合だけを行う）。
- [x] 標準ライブラリ検査を実装する（VM不要）。accepted の tests と replacements の `import` 対象を静的に列挙し、replay profile の `stdlibModulesPath`（テンプレート内 `python3` の `sys.stdlib_module_names` をタスク8で取得した固定一覧。`profileHash` の対象）と題材内モジュールの集合に照合して、外にあるモジュールがあれば VM 未作成の blocked。実行中に依存を取得しない。
- [x] 役割ごとに新規VMを順に使う。mode で分岐する: candidate-comparison は before → after、reproduction-only は before のみ、recheck は after のみ（仕様04「修正候補がない初回は before のみ実行」、タスク6の照合と対称）。各役割は `New-VerificationSandbox replay-<role>` → Copy/Confirm（`/home/agent/workspace/source`）→ 固定argv `["python3","-m","unittest","discover","-s",".verification-tests","-p","test_*.py","-v"]` を `WorkingDirectory=/home/agent/workspace/source`、Environment は profile の最小辞書、出力署名 `{pattern: 'Ran \d+ tests? in', stream: stderr}` で実行（replaySeconds）→ 停止確認。before の停止未確認なら after を作らず返す。
- [x] 各コマンドを `control/replay/<role>/<commandId>.json` へ `replay-record.schema.json` どおり保存する（runId、role、sandboxId、profileHash、effectiveSettingsHash、templateDigest、sourceManifestHash、inputManifestHash、testsManifestHash、argv、workingDirectory、startedAt、finishedAt、exitCode、stdout/stderr のパスとhash、timedOut、outputExceeded、stopVerified、transportVerified、activationRecordPath/Hash、limits）。
- [x] `New-VerificationReplayNotRun` を実装する（status=not_run、sandboxes=[]、before/after/allStopped=null、failure に上流段階）。作成成否不明は not_run へ丸めず `failure.reason=creation-unresolved` で incomplete。
- [x] `tests/ReplayV3.Tests.ps1` で、偽sbx が before（VM名 `-before`）に終了1・after（`-after`）に終了0 を返すケースで `completed`（合否は付けない）、reproduction-only で after VM が作られないこと・recheck で before VM が作られないこと、unittest の署名が stderr に無い出力（接続失敗の模擬。stdout にだけ署名がある場合を含む）で incomplete、before 0 の非再現、after 非0、テスト集合が before/after で異なる入力の拒否、after の差分が replacement 一覧に無いパスを含む入力の拒否、replacement が after に反映されていない入力の incomplete、内容が同一の replacement を含む入力が incomplete にならないこと、recheck で差分照合を行わないこと、標準ライブラリ外 import の VM 未作成 blocked、別 digest の profile 拒否、before 停止未確認で after 未作成かつ incomplete、偽成功文字列（stdout に "OK" だが終了1）が終了コードを覆さないこと、出力洪水で incomplete、時間超過で timed_out かつ停止コマンドが `phase=cleanup` で発行されること、`not_run` の書式、各役割の sandbox id が相異なり `effectiveSettingsHash` が一致すること、当該名以外への stop が0件であることを確認する。
- [x] コミットする。

逸脱記録: 実体に合わせる調整 / 採用 / pilot-source を source・tests・replacements に分ける（複製元は pilot-source/source）、replay-record に commandId を加える、仕様04に従い新規VMの activationRecord と runId・sandboxId・実効設定の対応を搬入前に確認する、status はテスト集合の相違と差分照合の不成立を incomplete・activationRecord 不一致を blocked・作成成否不明で全体期限到達を timed_out（仕様02「時間超過なら timed_out を優先」）とする、not_run（recheck 以外）と入力検査前の blocked では mode を null にする、作成直後に既出の sandbox id を拒否する sandbox-reused を置く（仕様04「各役割の実行環境が新規であることを id で照合」の実現）、before に停止未確認以外の問題（署名なし・出力超過・時間超過）があっても after を作らない、ProposalResult の origin と recheck の有無の結び付けを追加検査する（2026-09-15 タスク5 実装者報告、型判定候補を主担当が調整と判定）
逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / 本タスクの「Environment は profile の最小辞書」はタスク3の runtime-profile.schema.json に対応項目が無く計画内で食い違うため、Replay の定数の空辞書で固定する（profile に environment を加える schema 変更は、タスク8a で環境変数が必要と分かった場合に設計の変更として扱う）（2026-09-15 タスク5 実装者報告）
...
- [x] `result.schema.json` に v3 定義（schemaVersion、runId、status、summary、sourceState、baselineState、proposalVerdict、replayVerdict、checks、findings、artifacts、unverified、execution、previousRunId、runRoot、sourceManifestPath、scope、limitations。`additionalProperties:false`）を足し、v1 定義は保つ。`execution` は proposalCreated、replayCreatedCount、proposalStopped、replayAllStopped、failureStage、exitCodeForCli、pilotInputId、pilotInputPath、pilotInputHash を持つ。
- [x] 照合手順1〜7（仕様03）を実装する。順序は schemaVersion/runId → activationRecord と profileHash/effectiveSettingsHash/sandboxId/daemonInstance（before/after の effectiveSettingsHash 一致は before と after の両方がある mode=candidate-comparison のときだけ。reproduction-only は before のみ、recheck は after のみを照合）→ manifest 期待hash と現物（source・baseline・recheck・proposal） → 原本再列挙 → accepted・replay input・testsManifestHash → （mode=candidate-comparison のときだけ）before/after の差分が replacement 一覧に含まれ、各 replacement が適用されていること → 外側記録（runId/role/VM id/argv/時刻/hash/transportVerified/停止）→ 停止記録の必須化 → 観測分類。証拠パスは control 内だけを許す。
- [x] status の優先順位（timed_out → incomplete〈作成不明・作成済みの停止未確認・基準版改変〉→ blocked → source_changed → incomplete〈その他〉→ completed）と、Proposal/Replay の status 写像、`not_run` の上流追従を実装する。scope 付与前に pilotInputHash と記録現物、sourceRoot/sourceManifestHash を再照合し、`execution` に pilotInputId/Path/Hash を載せる。
- [x] `replayVerdict` の観測分類（candidate-supported / not-reproduced / still-failing / reproduced / current-pass / current-fail / undetermined）と、completed 時の CLI終了値（0/1）を `execution.exitCodeForCli` として返す。
- [x] `New-VerificationFailureResult` を実装する（runId/runRoot/sourceManifestPath/previousRunId=null、sourceState/baselineState=unreadable、proposalVerdict=null、replayVerdict=undetermined、空配列、execution は proposalCreated=false・replayCreatedCount=0・停止値null・失敗段階・pilotInput 3項目は判明していれば値/なければ null、判明済みVM情報があれば保持）。
- [x] `control/result.json` を CreateNew で一度だけ保存し、既存があれば例外にして保全する。
- [x] `tests/ResultV3.Tests.ps1` で、正常（candidate-supported、終了0）、still-failing（終了1）、reproduction-only、recheck の current-pass/current-fail、架空 commandId、別 sandboxId/世代、before/after の effectiveSettingsHash 不一致、自己申告 pass だけの入力、基準Git改変（incomplete）、原本更新（source_changed）、出力欠落、同一テストでない比較、after の差分が replacement 一覧を超える入力、停止未確認（incomplete かつ blocked より優先）、not_run 追従、失敗結果の null 書式、既存 result.json 保全、schema 適合（execution の pilotInput 3項目を含む）を確認する。
- [x] 既存 `Result.Tests.ps1` の成功を保ち、コミットする。

逸脱記録: 実体に合わせる調整 / 採用 / result.schema.json の v3 定義で findings・artifacts（再検証のテストに previousRunId を持たせ由来を明記）・checks の項目を仕様03の記述から定め、unverified は「段階:理由: 説明」の文字列にする（最上位キーは仕様03のまま、message キーは足さない）、失敗結果の runId は RequestContext に確定値があれば返し無ければ null（仕様03「RequestContextは確定したrunId…または各null」）、daemonInstance の run 内不一致・baseline manifest の不読・照合で見つけた証拠不正は incomplete（その他）、作成記録があるのに結果に無い VM は作成不明として blocked より優先の incomplete、照合の並びと停止記録の探索は role ごとのファイル一覧から行う、runId を持たない記録の読込みとツリー列挙は Result.psm1 の局所関数（2026-09-16 タスク6 実装者報告、型判定候補を主担当が調整と判定）
逸脱記録: 実体に合わせる調整 / 採用 / 修正ラウンド1でタスク5・6の重複した補助（値と limits の取り出し、失敗例外の送出、問題の優先付け、検査済み JSON、ツリー列挙、差分計算、提案 artifacts の区分と manifest 照合、size を含む置換差分照合、daemonInstance 検査）を RequestCopy.psm1 の公開補助へ集約し Proposal・Replay・Result から呼ぶ（調整 (a) の適用、照合側で size 比較が抜けていた食い違いの解消）、共通の失敗送出の名前を Invoke-VerificationFailure にする、照合は PreparedRunV3.settings の profile パスから能力証拠を検査し profileHash・templateDigest・effectiveSettingsHash を activationRecord と再実行記録に照合する、Replay の proposal manifest のキー欠落の reason を proposal-manifest にする（status は blocked のまま）（2026-09-16 タスク6 レビューと修正ラウンド1）
逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / ProposalV3.Tests.ps1 の停止猶予の既定 5 秒が Issue-0147 修正後のホスト起動費用を含む現環境の偽sbxに足りずケース11が停止未確認で失敗したため、既定を 30 秒にする（停止未確認を作るケース12は 5 秒を明示、修正前コミットでも同じ失敗を再現）（2026-09-16 タスク6 修正ラウンド1）
...
- [x] 入力読込（`Test-VerificationJsonDuplicateKeys` で重複キー検査）→ `startedAt` 確定 → `New-VerificationRun`（失敗は `New-VerificationFailureResult`）→ `Test-VerificationRuntimeProfile`（replay は常に、proposal は非recheck時）→ `Acquire-VerificationPilotLease` → `Invoke-VerificationProposal` → ready なら `Invoke-VerificationReplay`、非ready なら `New-VerificationReplayNotRun` → `Complete-VerificationRun` → `finally` で作成済みVMの停止確認（未停止があれば停止フェーズの予算〈台数分〉を組んで `Stop-VerificationSandbox` を再試行）と `Release-VerificationPilotLease`。準備後の例外は判明済みVM情報を RequestContext に足して失敗結果へ。
- [x] 開始時に scope と acceptedLimitations（clipboard文字列書込の可能性・プロセス数上限なし・デーモン切断は未確認）と runRoot を stderr に表示する。clipboard の読取・退避・復元・消去はしない。
- [x] 全体 deadline を単調時計で監督し、到達後は新規VM作成・搬入・execを行わず停止処理だけ停止フェーズの予算まで許す。
- [x] `tests/CliV3.Tests.ps1` で、偽sbx と `tests/fixtures/pilot-source`（ケース領域の独立 Git 作業ツリーへ複製して初期コミットしたもの）を使い、正常往復（stdout JSON 1件・終了0）、still-failing（終了1）、固定入力不一致（blocked・終了2）、デーモン停止（blocked・理由に通常起動）、Lease競合（blocked）、提案非ready→replay not_run、途中失敗時の Lease 解放と作成済みVMの停止発行、deadline 到達後に stop だけが台数分の予算で発行されること、`-StopRecorded` が記録済みIDだけを停止し記録外のVMに触れないこと・Lease競合時に何もしないこと・入力が recovery-input の schema で検査され不足キー（schemaVersion 欠落を含む）で終了2になること・出力が recovery-result の schema に適合すること、開始直後の stderr に runRoot が出ることを確認する。
- [x] `Run-IndependentTests.ps1` に v3 の7群（RequestCopyV3、ExecutionV3、SbxRuntimeV3、ProposalV3、ReplayV3、ResultV3、CliV3）を足し、出力を「11 suites passed」にする。README に v3 の使い方、前提（利用者起動のデーモン）、既知の制約（自動停止・clipboard例外・デーモン切断未確認・pilot限定・別ログオンセッションからの同時実行は排他できない・作成記録前に失われたVMは復旧操作の対象外・evidence に使った実験記録は追記しない〈追記時は profile 再生成〉・残置VMの後片付けは利用者承認の `sbx rm`）、`profiles/evidence-checks.md`（verified の意味）と `profiles/replay/` への参照を書く。
- [x] 11群成功を確認してコミットする。

逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / -StopRecorded の入力は本タスクの「schemaVersion=3 を添えて検査」と試験項目の「schemaVersion 欠落で終了2」が計画内で食い違うため、入力の schemaVersion をそのまま recovery-input.schema.json で検査し CLI は補わない（SettingsV3 も縮約入力も schemaVersion=3 を持つ前提）（2026-09-16 タスク7 実装者報告）
逸脱記録: 実体に合わせる調整 / 採用 / -StopRecorded が入力の schema 違反や Lease 競合で復旧操作に入れないときは stdout に何も書かず終了値2と stderr の理由を返す（recovery-result を作れないため）、runRoot は準備の直後で VM 作成の前に stderr の最初の行に出す、例外から作る失敗結果の理由は「符号: メッセージ」、finally の停止確認は作成記録を正本にする（2026-09-16 タスク7 実装者報告、主担当が調整と判定）
逸脱記録: 計画の範囲外の既存欠陥 / 採用 / 全体期限の後の sbx daemon status の起動拒否を SbxRuntime が daemon-not-running（blocked）と表示していた欠陥（タスク3由来）を timed_out に直す（実装時レビューの Important を直す前提で、同一領域を触る本タスクに統合。検出元は CliV3 の実装者報告で、既存の SbxRuntimeV3・ProposalV3・ReplayV3 の試験は期限後の呼び出しを持たず捕捉しない）、あわせて期限後に呼ばれた Proposal・Replay は VM を作らず timed_out の型付き結果を返し、SbxRuntime の実行前の維持確認は期限到達を incomplete に言い換えない（2026-09-16 タスク7 修正ラウンド1）
...
- [x] **8b: 公開操作の実証**。8a で生成した profile が `Test-VerificationRuntimeProfile` を通ることを確認し、8b 用に新しく準備した run（PreparedRun・runRoot・Settings）と probe が外側で組んだ入力（`pilot-source` の before と replacement 適用後の after、tests）を使って SbxRuntime の公開操作を直接呼ぶ: `Acquire-VerificationPilotLease` → `New-VerificationSandbox replay-before` → Copy/Confirm → unittest の `Invoke-VerificationSandboxCommand` → `Stop-VerificationSandbox`（`stop` → `ls` 確認 → 保持ジョブ停止の順）→ after を同様に → Lease 解放。before 非0・after 0、各役割の新規 id、activationRecord の生成、外側 `stop` による停止確認、他VM（試験VM2台）の一覧上の状態不変を取る。`Invoke-VerificationReplay` は呼ばない（上記の範囲の説明どおり）。
- [x] 8a で取得した `python3 --version` と `sys.stdlib_module_names` の一覧を固定ファイル化し、replay profile の `stdlibModulesPath/Hash` に載せる（3.10 未満なら一覧の取得方法を見直し、記録する）。
- [x] 差し替え後に `Run-IndependentTests.ps1` を再実行して11群成功を確認し、期待値の修正が必要なら同時に直す。
- [x] 実験記録へ結果と限界（1回の成立、負荷なし、デーモン切断は未注入、`Invoke-VerificationReplay` の統合は未実証、他VMの非停止は停止中のVMに対してのみ確認）を書き、replay profile の activationEvidence を固定版・固定設定の能力試験記録として保存する。コミットする。


逸脱記録: 実体に合わせる調整 / 採用 / 8a の足場は probe の totalSeconds を 7200 にし -HoldSeconds・-AutoStopWaitSeconds を引数にする（実 run には使わない試験足場の値）、cp が失敗した場合に搬入先の親を作って1回だけ再試行する（実機では未発動）、daemonDisconnect の evidencePath は偽 fixture の試験スクリプトではなく 2026-09-14 の能力試験記録を指す、mcp ls の signed_in_as は追跡する記録へ転記しない、実験記録を profile より先にコミットする（profile が記録の SHA256 を固定するため）（2026-09-16 タスク8a 実施者報告）
逸脱記録: 実体に合わせる調整 / 採用 / 偽sbxの `execClientKilled` と `unittestOk` は synthetic のまま残す（外側からの強制終了は偽sbxでは終了コードを創作にしかできず、成功時の unittest は 8a の承認範囲外で未観測。実測していないものを実測として記録しない）。差し替えたのは exec 127・exec 7・unittest の失敗出力（2026-09-16 タスク8a 実施者報告）
逸脱記録: 対象外 / 対象外 / 主担当がタスク8の要件抜き出しファイル（task-8-brief.md）を作らずに委譲した（タスク1〜7の分だけ作成していた）。実施者は不在を報告したうえで委譲文の要件一覧と共通制約を要件として実施し、承認範囲の逸脱はない。8b の委譲前に同ファイルを作成した（2026-09-16、主担当の手順の誤り）

逸脱記録: 事実誤り・期待値の陳腐化の訂正 / 採用 / Settings の replayProfilePath はディレクトリ `profiles/replay/` ではなく profile ファイル `profiles/replay/profile.json` を指す（Test-VerificationRuntimeProfile は profile ファイルの親ディレクトリを相対パスの基準にするため、ディレクトリを渡すと必ず拒否される）。本計画の該当箇所の表記が実装と食い違っていた（2026-09-16 タスク8b 実施者報告）
逸脱記録: 実体に合わせる調整 / 採用 / 8b は 8a の probe を拡張せず兄弟スクリプト tests/Invoke-SbxPublicOperationProbe.ps1 を新規作成（8a は固定argv直接と自己強制終了、8b は公開操作だけで責務が逆）、Lease は計画本文どおり run に1回だけ取得する（Mutex の再入を避ける）、題材の複製先は runId 発番前に決まる足場 id を使う、足場専用の引数を追加、入力 manifest は Replay を呼ばないため自前形式にして Replay の証拠に流用しない、unittest 成功時の偽sbx応答を 8b の実測原文へ差し替える（2026-09-16 タスク8b 実施者報告）
逸脱記録: 対象外 / 対象外 / 実機の観測で計画の前提と違った点: 外側 stop の約30秒後に当該VMの自動停止行が出る（停止の証拠は stop 発行以後の stopped runtime container 行で判定するため判定に影響しない。docs/reference/sbx-sandbox-runtime-facts.md へ追記）。タスク3の懸念「MSIX 版 pwsh の孫がジョブに入らない」は実機では起きず、偽sbxの構成に固有（Issue-0147 へ追記）
逸脱記録は実施後に残す。

## タスク9: 最小モデル往復（個別承認、認証方式の実測が前提）

**2026-09-17の訂正承認**: 初回実測後の利用者の「1で」により、既定キットのまま2ホスト・443番限定へ訂正し、policy checkの正常拒否（終了1）の処理を修正する。実測応答による回帰試験・既存11群・独立レビューを経て新規2台で再試験する。既存6台を停止保全。実装項目と検証条件は同日の初回試験記録「修正案」1〜4に従い、以下の過去の全ポート記載は現在の期待値として使わない。

**2026-09-17の実測停止点**: OAuth登録後にproposal probeの1台目を実施したが、実効規則が2ホストの443番限定であることを確認し、全ポートという前提の誤りで中断した。対象VM停止・既存5台保全を確認済み。Issue-0150を再開し、期待値の訂正とpolicy check終了1の処理修正を提案中。2台目・モデル往復は未実施。正本: `docs/records/experiments/2026-09-17-v3-task9-proposal-probe-first-attempt.md`。
