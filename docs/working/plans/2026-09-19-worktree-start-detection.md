...
結果（2026-09-19 実施。写経スクリプトは scratchpad の `worktree-detect/detect.sh`）:
- 場面 A: 除外（worktree 側 in_progress と写し completed が同時刻 2026-09-10 10:00）。期待どおり
- 場面 B・C・D: 候補（C 09-15 → B 09-12 → D 09-11 の順）。C は取り込み後の再開で写し（completed 09-09）より新しく、除外されない。期待どおり
- 場面 E: 特定不可（ブランチなし。プロジェクトの外の detached HEAD）。期待どおり。初回は写経スクリプトの区切り文字の誤り（bash の read が連続タブを1つに扱う）で表示がずれたため、スクリプトを直して再実行した。文面の欠陥ではない
- 場面 F: 特定不可（handoff なし）。期待どおり
- 場面 G: 対象外（ready-for-next-cycle）。期待どおり
- Step 4（場面 F から実行）: 候補は main（09-19）・C・B・D、除外は A、特定不可は E。期待どおり
- Step 5（worktree 1つ）: 「worktree数: 1」で検出を終える。期待どおり
- Step 6（実環境）: 取り込み済みの5件をすべて除外し候補0件。既定ブランチは master（推定）。プロジェクトの外の Codex 側 worktree は直接読めた。期待どおり
...
照合: 食い違いなし（2026-09-19。実体を読み直して確認）
- 観点1 仕様のスナップショット性: ADR-0202 Decision 3 の前置き・8項目が op-read.md の節の前置きと 1〜6 項目・手順 2/3/5 の接続に一致。計画レビューで明確化した「その worktree のフォルダにある handoff」は ADR の「他のworktreeそれぞれで、そのブランチのhandoff」を具体化したもので、決定の変更ではない。Phase 0 の1句は ADR の「両分岐に共通する1句」を分岐の前に置いたもので一致。サイズ警告への対応は ADR-0203 に記録済み
- 観点2 規範の書き戻し: 新設の検出（op-read の節）は ADR-0202、inline-fallbacks.md への移動は ADR-0203、操作一覧の語句は op-read の節への索引で、いずれも決定に記載あり
- 観点3 数値・完了基準: start-work SKILL.md 18,992 バイト（ADR-0203 と一致）。op-read の手順番号は不変で、op-finalize.md の「`op-read.md` 手順 4」参照は有効
- 観点4 経路の閉じ: (a) handoff あり→手順2で検出→手順5の要約に添える→Phase 0 の1句で提示。(b) handoff なし→手順2で検出→手順3で結果を添えて返す→Phase 0 の1句で提示しPhase 1へ進むか判断。(c) worktree 1つ→前置きで終える（追加出力なし）。(d) read を単独で呼ぶ→手順5の要約と節の6項目で案内。二重発火: Phase 0 の1句と節の6項目は同じ案内（開き直しを促す）で、start-work 経由では同じ内容を1回提示すれば足り矛盾しない。記録の順序: 検出は記録を書かない
- 観点5 引用の整合: op-read の（ADR-0202）は全角括弧に番号のみ。ADR-0203 の「発火時に毎回必要な内容の退避は認めない」への適合は、インラインフォールバックの内容が superpowers 不在時にだけ読まれることで成立（Phase -1 の不足報告が発火条件）
