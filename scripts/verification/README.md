...
- 提案用はCodex収録済みの公式codex版、再実行用はshell版の固定digestを使います（ADR-0201）。`--template`へshell版を明示すると、agentをcodexにしてもCodex本体は追加されません。画像内の版・絶対パス・非対話起動の実測を、ホストCLIの情報で代用しないでください。
