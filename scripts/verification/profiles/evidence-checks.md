...
replay側の試験A〜C（否定試験・負荷試験）と、proposal側の負荷試験Cの再実施は実装計画の承認に含めない。proposal側で数秒で取れる試験A・Bのゲスト側確認は、ADR-0197によりタスク9で取り直す（下表の (9)）。その他の再実施を望む場合は承認区切りの拡大として利用者が指示する。

## 条件 × 役割

「同記録の節」は (既存) の記録 `2026-09-14-v3-capability-test-methods.md` 内の節名。

| 条件名 | replay（agent=shell、`deny *`） | proposal（agent=codex、`auth.openai.com:443`・`chatgpt.com:443` だけを許可） |
|---|---|---|
...
| `modelEndpointAllowOnly` | 不要 | (9) `policy ls <VM名> --json` の有効なallowが実測の13件（HTTPS系10件は`:443`、Ubuntu系3件は`:80`）に限られ、残る11ホストへのポート番号なしdenyがあり、未知の追加allow・ワイルドカード・拒否漏れが無いことを確認する。`policy check network --sandbox <VM名> --json` で承認2ホストそれぞれの443番が許可、8443番・残る11宣言先・外部対照1件が拒否されることを確認する。正常許可は終了0/allowed=true、正常拒否は終了1/allowed=falseとして、応答の対象VM・接続先・型も照合する。これは規則一覧と代表ポートの対照であり、全ポートの実通信試験ではない |
