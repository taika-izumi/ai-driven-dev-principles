...
function Invoke-ProbeSetup() {
    ...
    # 固定 argv（New-VerificationSandbox と同一。agent は replay 役と同じ shell）
    $createArgv=@('create','shell','--name',$script:State.name,'--cpus','2','--memory','2g','--no-share-skills','--deny-network','*','--template',$script:TemplateDigest)
    ...
    $handle=@{runId=$script:State.runId;role='probe';name=$script:State.name;id=$script:State.id;createdAt=$script:State.createdAt;profileHash=$null;effectiveSettingsHash=$null;activationRecordPath=$null;activationRecordHash=$null}
...
function Invoke-ProbeQueries() {
    ...
    [void](Write-ProbeObservation '05-queries' @{
        checkedAt=(Get-VerificationUtcNow);inspect=$inspect.value;policy=$policy.value;settings=$settings;mcp=$mcp.value;list=@($list)
...
if($Resume){
    ...
    $script:State=@{
        ...
        # VM 名の規則（実装計画）: iv-<runId の先頭8桁>-<role>。8a は role=probe。
        name=('iv-'+$runId.Substring(0,8).ToLowerInvariant()+'-probe')
...
try{
    ...
    $script:State.lastError=([regex]::Replace([string]$_.Exception.Message,'\s+',' ')).Trim()
    try{Save-ProbeState}catch{}
    Write-Host ''
    Write-Host "probe が中断した: $($script:State.lastError)"
    Write-Host "  runRoot: $($script:State.runRoot)（再開は -Resume `"$($script:State.runRoot)`"）"
    throw
}
