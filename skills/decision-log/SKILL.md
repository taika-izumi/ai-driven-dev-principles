...
**設計を確定させた ADR ドラフト（設計文書ファイルを作らない拡張で、ADR が設計文書を兼ねる型）のコミットは spec 確定点 (c) にあたる。** コミットの前に `pre-finalization-review` の提示操作を呼んで確定前レビューを提示すること（ADR-0080）。設計を確定させるものではない ADR（記録のみの決定）はこれに当たらない。
