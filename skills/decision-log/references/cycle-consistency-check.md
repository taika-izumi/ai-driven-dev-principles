...
**重複実施の抑止**: 同一サイクルで本検査を実施済み（消化記録に `cyclecheck=` の行がある）かつ以後の差分が無い場合に限り、再実施を省略する。差分がある場合、観点 4（発動経路と記録順序の整合）は差分に関わる規範のみの再検査でよいが、観点 1（仕様のスナップショット性）・2（規範の書き戻し）・3（数値・完了基準の整合）・5（引用の整合）は照合対象文書の全体を読み直す。本検査以外のレビュー（独立レビュー・最終レビュー等）は同等検査とみなさない。昇格処理そのものが生む差分（ADR の Status 更新・決定インデックスの行更新・close する課題・handoff への記録）は「以後の差分」に数えない（検査→昇格→再検査のループに停止条件を与えるため）。
