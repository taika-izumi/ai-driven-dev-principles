...
- **(a) 推奨の由来を明示する**: 推奨が上記の既定（判定に迷う場合はフル側へ倒す規定）に由来する場合は、**その旨を明示し**、判定を迷わせた両側の事情を提示する。内容判定による推奨と既定による推奨とが、提示を読んだだけで区別できるようにする。反復提示に準用する場合、「上記の既定」は `references/iteration-norms.md`「指摘反映後の反復」でフル巡側・設計骨格側へ倒すことを命じる各既定（2 段型の最終分岐・骨格安定の判断・レイヤ振り分け）と読み替える（レイヤ既定を経由して締めモードの先行判定が非発火となった場合も既定由来に数える）
