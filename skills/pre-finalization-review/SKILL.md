...
2. **委譲プロンプトの組み立て**: 観点ごとに独立サブエージェントへ委譲する。委譲プロンプトは `subagent-dispatch` スキルで組む（B 群「検査・監査・走査」が発火し、全数走査・母数明示・実証なし指摘の不採用が制約に入る）。**レビュアーのモデルは前工程（成果物を作った側）と変える**（根拠エントリ由来の運用ヒント。Issue-0034）
