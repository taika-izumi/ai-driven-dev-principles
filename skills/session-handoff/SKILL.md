...
- プロジェクトの目的・方針: <対象ルート・正本のパスと節・参照した内容の根拠>
...
`start-work` の `references/project-purpose.md` の参照結果に従う。「関連ドキュメント」に所在・適用範囲・参照内容の根拠を、「進行中のタスク」または「既知のブロッカー・懸念」に未確定・未反映事項と影響を記載する。目的本文を毎回複写せず、正本未整備と目的未確定を区別する。前回参照版は履歴の識別に使い、次回の目的をその版へ固定しない（ADR-0190）。

## 「本サイクル」の定義

「本サイクル」とは、前回 cycle-reset から次の cycle-reset までの作業単位を指す（ADR-0087）。本スキルのほか `pre-finalization-review`（確定点での提示）と `decision-log`（サイクル全体整合検査）がこの定義を参照する（各所で再定義しない）。
...
複数の操作から共用される規範。操作ファイルが名指しした時点、または下表の「読むとき」に当たった時点で読む（毎回は読まない）。
