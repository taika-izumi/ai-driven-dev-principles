...
`review=` は確定点（spec 確定点 / plan 確定点）を通過したマイルストーンにのみ書く（ADR-0080）。それ以外のマイルストーンでは省略してよい。確定点を通過したマイルストーンは、**名称に `spec 確定点 (a)` / `spec 確定点 (b)` / `spec 確定点 (c)` / `plan 確定点` のいずれかを含める**（read の欠落検査が対象行を識別できるようにするため）。
