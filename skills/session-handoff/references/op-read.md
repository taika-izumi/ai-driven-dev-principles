...
手順:
継続要約の提示と目的に依存する判断の前に、`start-work` の `references/project-purpose.md` に従って現在の目的・方針を取得する。start-workで同じ内容を取得済みなら再利用し、単独readの場合もこの取得を省かない。handoffの前回参照内容・未反映事項と照合する（ADR-0190）。

判断の分担が記録されている場合は、要約提示前に `start-work` の `references/decision-delegation.md` に従って正本・承認基準・変更や撤回を読み直す。有効な同一作業の許可は取り直さず、矛盾・根拠喪失は影響する判断に限って確認する。末尾の継続確認は作業を再開するかの選択であり、許可の取り直しとは区別する（ADR-0133）。
...
6. 「節目ごとの確認記録」を検査する（ADR-0057）: 「完了済みタスク」にあるマイルストーンで確認の記録の行が無いもの、または `ADR=` / `worklog=` の記載が欠けた行があれば、未記録として呼び出し元（start-work）へ報告する。確定点（spec 確定点 / plan 確定点）を通過したマイルストーン行に `review=` が欠けている場合も、同様に未記録として報告する（ADR-0080）。名称に `Accepted 昇格` を含むマイルストーン行に `cyclecheck=` が欠けている場合も、同様に未記録として報告する（ADR-0092）。いずれの検査も、確定点ラベル・`Accepted 昇格` の照合対象は行頭からコロンまでのマイルストーン名に限る（フィールド値・括弧内の文字列は照合に用いない。途中経過行が本文で確定点を参照しても誤検知しないため）。
