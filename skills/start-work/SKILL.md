...
**確定点を通過したマイルストーンでは、先に確定前レビューの提示または委任に基づく通知を行い、個別回答または有効な委任による判断結果が確定してから update を呼ぶこと**（`review=` の値は判断の根拠と結果が揃ってから確定するため）。順序が前後して update を先に済ませてしまった場合は、回答を得た時点で当該行へ `review=` を追記する。
