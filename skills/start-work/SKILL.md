...
- **(a) 推奨の由来を明示する**: 推奨が上記の既定（判定に迷う場合はフル側へ倒す規定）に由来する場合は、**その旨を明示し**、判定を迷わせた両側の事情を提示する。内容判定による推奨と既定による推奨とが、提示を読んだだけで区別できるようにする。反復提示に準用する場合、「上記の既定」は「4. 指摘反映後の反復」の「どの分岐にも該当しない場合・判定に迷う場合はフル巡側へ倒す」既定と読み替える
