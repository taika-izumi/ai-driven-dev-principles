...
**停止判定（ADR-0107 決定 2）**: 反復の停止は件数・巡数でなく指摘の分類で判定する。「成果物の実体への新規の設計欠陥がゼロ、かつ残指摘が直前の修正に由来する機械検証可能な型のみ」となった巡を実質収束とみなす（第 1 経路）。実質収束は確定前工程の停止判定であり、実装時の独立レビューを代替しない（補完関係）。判断材料として、各巡の指摘を対象レイヤ（以下の 2 分類）で「設計骨格（決定の構造・条件・機構そのものの欠陥。判定条件・分岐・記録機構と、規範を発火点・記録先へ接続する配線の新設・変更・削除を含む——機構を減らす設計縮小も骨格に触れる改訂である）」と「周辺（文書間整合・写し漏れ・修正伝播・表現精度）」に振り分け、迷う場合（指摘・改訂のいずれの判定でも）は設計骨格側へ倒す（振り分けは提示に添える判断材料であり、記録・引き継ぎの機構は置かない）。
