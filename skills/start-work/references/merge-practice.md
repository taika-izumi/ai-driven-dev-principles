...
`start-work` の完了処理の発火点（Phase 2 マッピング表・作業前の確認の条項・セッション終了処理）から読まれる。
