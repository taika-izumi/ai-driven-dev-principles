...
サブエージェント（Task ツール等）へ作業を委譲する直前。タスクの型を問わず毎回。`start-work` の作業前の確認から配線されている。
...
常時適用・条件発火の各項目の根拠はそれぞれ単一プロジェクトの運用から得たもので、他プロジェクトへの一般化は保証されない。ただし常時適用の「実態優先の報告」と条件発火の「全数走査・母数明示」の 2 項目は、別プロジェクトでの試験適用でも効果を確認している。
