...
設計・計画・実装の独立レビューを委譲する場合は、`pre-finalization-review` の `references/review-criteria.md`に従い、全体目的・方針の根拠、対象の版・範囲、実体・実行条件と、担当する観点の定義の所在を渡す。全体目的との整合を確認対象に含める。確定前レビュー以外でも適用するが、この参照だけでレビューの起動・人数・方式を変えない（ADR-0185 / ADR-0186）。
