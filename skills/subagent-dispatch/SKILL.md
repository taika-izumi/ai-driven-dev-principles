...
| 長時間かつ書き込みを伴う作業を委譲するとき | 複数コミットへ分割し、中断時の復旧単位を作らせること | 長い作業に中断時の復旧単位を作らせる項目。再発 10 件のクラスタ由来（うち 9 件は人間の指摘を待たずに再発した手戻り）。全件が単一プロジェクト出所。世代: claude-opus-4-7〜claude-opus-5[1m]（出所: Issue-0033 項目4 / LoopForAlpha） |
