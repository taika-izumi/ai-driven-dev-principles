...
発火結果（記録したエントリ id、または記録ゲートによる棄却の旨）は、handoff の「節目ごとの確認記録」へ1行残す（ADR-0057。記載の形式と棄却時の記載義務は `session-handoff` update の消化記録手順が正）。
...
- ADR-0047: start-work の節目の確認への配線・全プロジェクト伝播
