...
5. **追記後に log.jsonl を読み直し、自行の id が他エントリと重複していないか検証する**（読み直し規範と整合している。ADR-0038）。重複を検出した場合、**自分が書いた行のみ id を再採番して書き直す**（追記専用原則の唯一の例外。台帳照合レコードが発生する前・自分が書いた直後の行の修復に限るため安全）
