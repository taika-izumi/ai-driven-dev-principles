...
- 「構造化質問ツールと同一ターンに書いた説明テキストがユーザーに表示されない」事象が確認されているモデルで動作している場合は、質問の重要度・自己完結の可否を問わず**構造化質問ツールを使用しない**こと。すべての質問を、説明と番号付き選択肢を含むテキストのみのターンで提示すること（事象確認済みモデル: Claude Fable 5 — Claude Code の AskUserQuestion で再現、2026-06〜2026-08 に複数回確認。自己完結形式の質問でも本文非表示の事故が再発したため全面不使用とした。新たなモデルで同事象を確認した場合はここに追記する。未確認のモデルでは、事象の発生を確認するまでこの対処には従わないこと）
